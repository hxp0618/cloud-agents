...
describe("Codex app-server runtime", () => {
  ...
  it("uses one Host-owned Workspace write tool without following symlink paths", async () => {
    await withFakeCodex("managed-dynamic-write", async (directory, _tracePath, environment) => {
      mkdirSync(join(directory, "actual"), { mode: 0o700 });
      symlinkSync("actual", join(directory, "linked"), "dir");
      const messages: RunnerMessage[] = [];
      environment.CLOUD_AGENT_CODEX_MANAGED_WRITE_RECEIPT_DELAY_MS = "250";
      const run = startProviderHostRun(
        codexInput(directory),
        null,
        (message) => messages.push(message),
      ...
      const managedFile = join(directory, ".cloud-agents-stage3-acceptance", "managed-dynamic.txt");
      for (let attempt = 0; attempt < 100; attempt += 1) {
        try {
          if (readFileSync(managedFile, "utf8") === "generated by managed tool\n") break;
        } catch {}
        await new Promise((resolveWait) => setTimeout(resolveWait, 5));
      }
      expect(readFileSync(managedFile, "utf8")).toBe("generated by managed tool\n");
      ...
      expect(messages).not.toContainEqual({
        ...
        payload: expect.objectContaining({
          ...
          itemId: "dynamic-write-1",
          status: "completed",
        }),
      });

      await expect(run.result).resolves.toMatchObject({
        output: { text: "managed file generated" },
      });
      expect(() => readFileSync(join(directory, "actual", "escape.txt"), "utf8")).toThrow();
      expect(messages.filter((message) => message.type === "artifact")).toEqual([
        {
          type: "artifact",
          artifact: {
            path: join(".cloud-agents-stage3-acceptance", "managed-dynamic.txt"),
            kind: "generated_file",
            contentType: "application/octet-stream",
            sourceRoot: "workspace",
          },
  ...
  it("reuses a recorded user-input resolution when the resumed question text changes", async () => {
    await withFakeCodex("user-input", async (directory, _tracePath, environment) => {
      environment.CLOUD_AGENT_TEST_USER_INPUT_VARIANT = "1";
      const run = startProviderHostRun(
        {
          ...codexInput(directory, { interactionMode: "plan" }),
          workload: {
            ...codexInput(directory, { interactionMode: "plan" }).workload,
            resumeSnapshot: {
              version: 1,
              sessionId: "session-1",
              turnId: "turn-1",
      ...
      await expect(run.result).resolves.toMatchObject({ output: { text: "staging" } });
    });
  });
...
readline.createInterface({ input: process.stdin }).on("line", (line) => {
  ...
  if (message.method === "initialize") {
    ...
    if (scenario === "resume" || scenario === "resume-with-recovery-metadata" || scenario === "resume-auth-failure" || scenario === "compact-rebuild" || scenario === "compact-auth-failure") send({ id: message.id, error: { code: -1, message: "unexpected thread/start" } });
      ...
      if (scenario === "managed-dynamic-write" && JSON.stringify(message.params?.dynamicTools) !== ${JSON.stringify(JSON.stringify(MANAGED_CODEX_DYNAMIC_TOOLS))}) process.exit(20);
      if (scenario === "managed-dynamic-write" && (message.params?.config?.features?.code_mode?.enabled !== false || JSON.stringify(message.params?.config?.features?.code_mode?.direct_only_tool_namespaces) !== JSON.stringify(["workspace"]))) process.exit(23);
      send({ id: message.id, result: { thread: { id: "thread-new" }, model: "gpt-test" } });
    }
    ...
    if (scenario === "approval") {
      ...
      send({ id: "user-input-rpc", method: "item/tool/requestUserInput", params: { threadId: "thread-new", turnId: "turn-1", itemId: "input-1", autoResolutionMs: null, questions: [{ id: "environment", header: "Environment", question: process.env.CLOUD_AGENT_TEST_USER_INPUT_VARIANT === "1" ? "Which deployment environment should be used?" : "Where should this run?", isOther: false, isSecret: false, options: [{ label: "Staging", description: "Use staging." }] }] } });
    ...
    } else if (scenario === "managed-dynamic-write") {
      send({ method: "item/started", params: { threadId: "thread-new", turnId: "turn-1", item: { id: "dynamic-link-1", type: "dynamicToolCall", status: "inProgress", tool: "workspace.write_text_file" } } });
  ...
  } else if (message.id === "dynamic-link-rpc") {
    if (message.result?.success !== false || fs.existsSync(require("node:path").join(process.cwd(), "actual", "escape.txt"))) process.exit(21);
    send({ method: "item/completed", params: { threadId: "thread-new", turnId: "turn-1", item: { id: "dynamic-link-1", type: "dynamicToolCall", status: "failed", tool: "workspace.write_text_file" } } });
    send({ method: "item/started", params: { threadId: "thread-new", turnId: "turn-1", item: { id: "dynamic-write-1", type: "dynamicToolCall", status: "inProgress", tool: "workspace.write_text_file" } } });
    send({ id: "dynamic-write-rpc", method: "item/tool/call", params: { threadId: "thread-new", turnId: "turn-1", callId: "dynamic-write-1", tool: "write_text_file", namespace: "workspace", arguments: { path: ".cloud-agents-stage3-acceptance/managed-dynamic.txt", content: "generated by managed tool\\n" } } });
    send({ method: "item/completed", params: { threadId: "thread-new", turnId: "turn-1", item: { id: "dynamic-write-1", type: "dynamicToolCall", status: "completed", tool: "workspace.write_text_file" } } });
    complete("managed file generated");
  } else if (message.id === "dynamic-write-rpc") {
    if (message.result?.success !== true || message.result?.contentItems?.[0]?.text !== "Wrote .cloud-agents-stage3-acceptance/managed-dynamic.txt") process.exit(22);
  }
});
`;
}
