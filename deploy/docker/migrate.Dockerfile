...
RUN mkdir -p /opt/cloud-agents/migrations \
    ...
    && rm /opt/cloud-agents/cloud-agents-migrations-000017.tar \
    && chmod 0555 /usr/local/bin/cloud-agents-product-migrate

ENTRYPOINT ["/usr/local/bin/cloud-agents-product-migrate"]
CMD ["--repository-root", "/opt/cloud-agents/migrations", "--manifest", "services/control-plane/migrations/product/000017/manifest.json"]
