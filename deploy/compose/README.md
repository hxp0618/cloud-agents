...
bottom of `.env.example`, then include the packaged override:

```sh
docker compose --env-file .env \
  -f docker-compose.yml \
  -f docker-compose.managed-agent.yml \
  up --build
```
...
CLOUD_AGENTS_REMOTE_WORKER_CAPABILITIES=docker,exec,files,network-dns-nft,preview,pty,ssh,workspace-snapshot,workspace-volume \
