...
- Status: Accepted by explicit owner approval on 2026-08-23
...
complete, and it does not turn a clean shutdown into crash-consistency evidence.
...
The owner has now explicitly approved using software shutdown/crash mechanisms instead of physical hard power removal.
Normal `poweroff` and `reboot` normally sync and unmount filesystems; they therefore prove lifecycle/restart behavior,
not abrupt-crash ordering. A no-sync crash primitive such as Magic SysRq `b`, or an equivalently documented abrupt
kernel/host reset, is the accepted stronger substitute.
...
3. The accepted durability closure must instead bind all of the following:
   ...
   - at least one owner-authorized bare-metal abrupt software-crash run observed from another host, with no application
     sync, filesystem unmount, clean shutdown, or `Lease.Close` before the crash;
...
4. Clean `poweroff`/`reboot` runs may supplement lifecycle smoke coverage but cannot, alone, satisfy the abrupt-crash
   item above or be described as physical power-loss evidence.
...
The existing bare-metal host-crash record supplies one candidate for the abrupt software-crash item: ext4
...
Future crash runs must use an owner-authorized target and an independent observer, establish the exact target and cleanup
scope before the crash, avoid production/shared data, and retain a bounded recovery record. This decision does not
...
- No ordinary `poweroff` result is promoted to abrupt-crash evidence.
