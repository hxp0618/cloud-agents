...
- Scope：固定提交 `3f92d8059cf51ab3bc995802f6bd85acfea5076a` 的 `services/control-plane` source、
...
- Source commit：`3f92d8059cf51ab3bc995802f6bd85acfea5076a`
...
- `golang.org/x/text v0.39.0` 现为 exact direct requirement：production evidence 的 NFC validator
  直接 import `golang.org/x/text/unicode/norm`；它不是 `replace` 或 fork；
...
| --------------------------------------------------------------- | ------------------------------------------------------------------ |
| Commit                                                           | `3f92d8059cf51ab3bc995802f6bd85acfea5076a`                         |
| Repository tree OID（Git SHA-1 object format）                   | `7cc2085b7b46bcb6c52873111f5d6d2dc97ce71b`                         |
| `services/control-plane` subtree OID（Git SHA-1 object format）  | `174853c911016e9309694bd4bc98c732f3ce94c1`                         |
| 120-file Control Plane `git ls-tree` manifest SHA-256            | `2a9ddd7802f4afbf43806689fe18d1adc76992f1a0a4f34f0c6b8ac72150d3f9` |
| 56-file tracked Go-source manifest SHA-256                       | `2c90de5769288f77e4c3156f41752f7dd8fbd595c371652297ff54d606e0b019` |

这里同时记录 Git object identity 与 SHA-256 manifest：前者绑定 repository/subtree DAG，后者为当前证据工具提供
算法独立的可重放文件清单摘要。source/import authority 完全来自 exact commit；本记录管理的四个 supply-chain
artifact 是随后刷新的派生物，不参与该 source commit identity。刷新时另有 repository-root `go.work.sum` 为
untracked concurrent-worktree state；`GOWORK=off`，它既不参与 Control Plane subtree，也不参与 module/import digest。
...
本次 direct-requirement 提升只改变 `go.mod` bytes；版本、`go.sum`、selected module list、module graph 和
production-linked module/package closure 摘要均不变。`go.sum` 是主模块实际需要的 checksum set；graph pruning 不保证它包含所有 graph-only module 的 content
...
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| [`dependency-lock.json`](../../../../services/control-plane/dependency-lock.json)     | 15-module selected graph、6/9 分类、SumDB/module/proxy/license/vulnerability evidence | `4bae3bfce955cdc1df34235b64308eb8bb6b411dacb65b56144039b143aeef7b` |
| [`sbom.cdx.json`](../../../../services/control-plane/sbom.cdx.json)                   | CycloneDX 1.6；15 components；root only depends on 6 production-linked components     | `d8ff0182bebbc3a4cedcba6534a2c2a7747ccbd642bcc0238251857d360b6fe5` |
...
漏洞 Snapshot 仍生成于 `2026-08-11T07:04:31Z`；结果是时点证据，不是永久安全证明。本次没有伪造新的
scanner/database timestamp：因 15 个 selected module 的 path/version/sums/proxy bits 与 29-package production
closure 均未改变，以下 exact evidence 被继承；它不是 2026-08-12 的 advisory refresh。
...
P1 C3 migration evidence contract 已进入固定提交，`dependency-lock.json` 与 SBOM 均记录
`source_tree_committed=true`、source commit、repository tree、Control Plane subtree、tracked manifest 和
Go-source manifest。non-test import closure、test、build 与 vet 都从该 exact source 重放；四个 refreshed
supply-chain artifact 不属于该 source commit，且没有参与 source/import digest。旧 vulnerability evidence
只按上节 exact-bits/closure 不变条件继承，不把它描述为本次重新扫描。
...
test "$(git rev-parse HEAD)" = "3f92d8059cf51ab3bc995802f6bd85acfea5076a"
test "$(git rev-parse 'HEAD^{tree}')" = "7cc2085b7b46bcb6c52873111f5d6d2dc97ce71b"
test "$(git rev-parse 'HEAD:services/control-plane')" = "174853c911016e9309694bd4bc98c732f3ce94c1"
...
test "$(sha256sum /tmp/cloud-agents-control-plane-ls-tree.txt | awk '{print $1}')" = \
  "2a9ddd7802f4afbf43806689fe18d1adc76992f1a0a4f34f0c6b8ac72150d3f9"
git ls-tree -r --full-tree HEAD -- services/control-plane |
...
test "$(sha256sum /tmp/cloud-agents-control-plane-go-ls-tree.txt | awk '{print $1}')" = \
  "2c90de5769288f77e4c3156f41752f7dd8fbd595c371652297ff54d606e0b019"
