...
> `G-INVENTORY` R3 与 `G-BASELINE-P0` R4（分别 supersede R2 与 R3）关闭，P1 已按 ADR-0007 从
...
| P0：Inventory                      | frozen-ref 全量输入分类、authority、characterization、provenance               | 已完成；Inventory R3 / Baseline R4 verified |
...
| Platform P0 | Inventory          | 已完成   | `G-INVENTORY`、`G-BASELINE-P0`                                                                        | Inventory R3 / Baseline R4 phase records 均为 `VERIFIED`                       |
...
| ------------------ | ---------------------------- | ----------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G-BASELINE-P0      | Platform Architecture DRI    | P0          | verified    | `CAG-G-BASELINE-P0-20260823-R4`：将 R2 的固定 ref legacy Synara/T3 characterization 与 greenfield Managed Host spec/negative/reference-host baseline 分别绑定到 Inventory R3 和当前 closure audit semantics；真实 Provider 未执行 |
...
以下 2026-08-08 决策继续有效；第 3 项的 Lease owner 已由 2026-08-10 接受的 ADR-0006 修订。Platform P0
现已关闭并允许进入 P1；不能据此恢复 M1 或提前实施 P2–P6：
...
M1 保持独立暂停现场。Platform P0 inventory 已完成；下一步不是只增加 `T3EnvironmentLease`，而是按 P1
先建立公共 contracts、SDK、数据/authority/security 基础，再实现 Managed Agent/Managed Host 两平面、
Standalone 部署和 Synara/T3 cutover。详细顺序只在
...
`G-INVENTORY` R3 与 `G-BASELINE-P0` R4 record 关闭；Inventory R1/R2 与 Baseline R1/R2/R3 仅保留为
`INVALIDATED` 历史记录。
