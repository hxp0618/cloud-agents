...
- 下一项：继续 BASE-M2，优先完成私有 Preview；随后完成短期 SSH 和网络策略实际执行及对应 Admin 管理。不得让 Admin 读取终端或文件内容。
...
| BASE-M2    | IN PROGRESS | bounded Exec、PTY/Files Grant/Gateway 与 Admin 诊断已真实接通；仍缺 Preview/SSH、策略执行和对应 Admin 管理 |
