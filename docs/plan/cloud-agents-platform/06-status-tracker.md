...
- 下一项：接通 generation-fenced outbound command、命令 deadline/幂等/回执，以及带影响确认、resourceVersion/generation、Operation/Audit 的 Admin 节点 Drain/Resume。
...
| BASE-M3    | IN PROGRESS | enrollment 与首条短期 mTLS 节点身份签发/Admin 元数据闭环已实测；尚缺轮换/吊销、outbound 重连/fencing、节点健康与 Drain/Resume |
