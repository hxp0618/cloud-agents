...
- 下一项：继续 BASE-M4，建立可信单租户、专用节点和共享不可信租户的实际隔离等级，并验证至少一条强隔离 runtime 的工具链/网络矩阵和限制界面。
...
| BASE-M4    | IN PROGRESS | direct Kubernetes、RemoteWorker 能力矩阵、单 Pool 聚合容量及多 Node 确定性选择已实测；仍需强隔离矩阵/限制界面 |
