...
func (client *Client) TerminateEnvironment(ctx context.Context, tenantID, projectID, environmentID, requestID, idempotencyKey string, body platform.UserEnvironmentTerminateRequest) (UserEnvironmentResult, error) {
	if err := validateLeasePath(tenantID, projectID, environmentID, requestID); err != nil {
		return UserEnvironmentResult{}, err
	}
	if err := common.ValidateIdempotencyKey(idempotencyKey, "/Idempotency-Key"); err != nil {
	...
	bodyBytes, err := platform.EncodeUserEnvironmentTerminateRequestJSON(body)
	if err != nil {
		return UserEnvironmentResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "POST", Path: "/v1/tenants/" + tenantID + "/projects/" + projectID + "/environments/" + environmentID + ":terminate", Headers: map[string]string{HeaderRequestID: requestID, HeaderIdempotencyKey: idempotencyKey}, Body: bodyBytes})
	if err != nil {
		return UserEnvironmentResult{}, err
	}
	if response.Status != 200 {
		return UserEnvironmentResult{}, client.problemError("managedAgentTerminateEnvironment", response)
	}
	value, err := platform.DecodeUserEnvironmentResponseJSON(response.Body)
	if err != nil {
		return UserEnvironmentResult{}, &ClientError{Operation: "managedAgentTerminateEnvironment", Status: response.Status, Cause: err}
	}
	if value.Value.ProjectRef.ID != projectID || value.Value.EnvironmentID != environmentID || value.Value.ObservedPhase != "terminated" {
		return UserEnvironmentResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/environmentId")
	}
	return value, nil
}
...
type TerminateUserEnvironmentServerInput struct {
	TenantID       string
	ProjectID      string
	EnvironmentID  string
	RequestID      string
	IdempotencyKey string
	Body           platform.UserEnvironmentTerminateRequest
}

func ValidateTerminateUserEnvironmentServerRequest(tenantID, projectID, environmentID, requestID, idempotencyKey string, body []byte) (TerminateUserEnvironmentServerInput, error) {
	if err := validateLeasePath(tenantID, projectID, environmentID, requestID); err != nil {
		return TerminateUserEnvironmentServerInput{}, err
	}
	if err := common.ValidateIdempotencyKey(idempotencyKey, "/Idempotency-Key"); err != nil {
		return TerminateUserEnvironmentServerInput{}, err
	}
	value, err := platform.DecodeUserEnvironmentTerminateRequestJSON(body)
	if err != nil {
		return TerminateUserEnvironmentServerInput{}, err
	}
	return TerminateUserEnvironmentServerInput{TenantID: tenantID, ProjectID: projectID, EnvironmentID: environmentID, RequestID: requestID, IdempotencyKey: idempotencyKey, Body: value}, nil
}
