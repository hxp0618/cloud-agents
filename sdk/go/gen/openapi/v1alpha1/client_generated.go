...
func ValidateGetAdminWorkerHealthServerRequest(tenantID, projectID, leaseID, requestID string, expectedGeneration int64) error {
	...
	if err := common.ValidateIdentifier(leaseID, "/leaseId"); err != nil {
		return err
	}
	if expectedGeneration < 1 || expectedGeneration > 9007199254740991 {
		return common.ContractError("INVALID_GENERATION", "/expectedGeneration")
	}
	return nil
}
func (client *Client) GetAdminWorkerHealth(ctx context.Context, tenantID, projectID, leaseID, requestID string, expectedGeneration int64) (WorkerHealthObservationResult, error) {
	if err := ValidateGetAdminWorkerHealthServerRequest(tenantID, projectID, leaseID, requestID, expectedGeneration); err != nil {
		return WorkerHealthObservationResult{}, err
	}
	path := "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/workers/" + leaseID + "/health?expectedGeneration=" + strconv.FormatInt(expectedGeneration, 10)
	response, err := client.roundTrip(ctx, Request{Method: "GET", Path: path, Headers: map[string]string{HeaderRequestID: requestID}})
	if err != nil {
		return WorkerHealthObservationResult{}, err
	}
	if response.Status != 200 {
		return WorkerHealthObservationResult{}, client.problemError("adminGetWorkerHealth", response)
	}
	result, err := platform.DecodeWorkerHealthObservationResponseJSON(response.Body)
	if err != nil {
		return WorkerHealthObservationResult{}, &ClientError{Operation: "adminGetWorkerHealth", Status: response.Status, Cause: err}
	}
	value := result.Value
	if value.TenantID != tenantID || value.ProjectID != projectID || value.WorkerID != leaseID || value.Generation != expectedGeneration {
		return WorkerHealthObservationResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/workerId")
	}
	return result, nil
}
...
type RuntimeProfileResult = common.ResponseEnvelope[platform.RuntimeProfile]
...
func (client *Client) ListAdminRemoteWorkerEnrollments(ctx context.Context, tenantID, projectID, requestID string, pageSize int, pageToken string) (RemoteWorkerEnrollmentPageResult, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, "", requestID); err != nil {
		return RemoteWorkerEnrollmentPageResult{}, err
	}
	if pageSize != 0 && (pageSize < 1 || pageSize > 200) {
		return RemoteWorkerEnrollmentPageResult{}, common.ContractError("INVALID_PAGE_SIZE", "/pageSize")
	}
	if pageToken != "" {
		if err := common.ValidatePageToken(pageToken, "/pageToken"); err != nil {
			return RemoteWorkerEnrollmentPageResult{}, err
	...
	if response.Status != 200 {
		return RemoteWorkerEnrollmentPageResult{}, client.problemError("adminListRemoteWorkerEnrollments", response)
	}
	value, err := platform.DecodeRemoteWorkerEnrollmentPageResponseJSON(response.Body)
	if err != nil {
		return RemoteWorkerEnrollmentPageResult{}, &ClientError{Operation: "adminListRemoteWorkerEnrollments", Status: response.Status, Cause: err}
	}
	for _, enrollment := range value.Value.RemoteWorkerEnrollments {
		if enrollment.Metadata.TenantRef.ID != tenantID || enrollment.Spec.ProjectRef.ID != projectID {
			return RemoteWorkerEnrollmentPageResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/remoteWorkerEnrollments")
		}
	}
	return value, nil
}
func (client *Client) CreateAdminRemoteWorkerEnrollment(ctx context.Context, tenantID, projectID, requestID, idempotencyKey string, body platform.RemoteWorkerEnrollmentCreateRequest) (RemoteWorkerEnrollmentResult, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, "", requestID); err != nil {
	...
	bodyBytes, err := platform.EncodeRemoteWorkerEnrollmentCreateRequestJSON(body)
	if err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "POST", Path: "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/remote-worker-enrollments", Headers: map[string]string{HeaderRequestID: requestID, HeaderIdempotencyKey: idempotencyKey}, Body: bodyBytes})
	if err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
	if response.Status != 201 {
		return RemoteWorkerEnrollmentResult{}, client.problemError("adminCreateRemoteWorkerEnrollment", response)
	}
	value, err := platform.DecodeRemoteWorkerEnrollmentResponseJSON(response.Body)
	if err != nil {
		return RemoteWorkerEnrollmentResult{}, &ClientError{Operation: "adminCreateRemoteWorkerEnrollment", Status: response.Status, Cause: err}
	}
	if err := requireResourceVersion(response, value.Value.Metadata.ResourceVersion); err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
	if value.Value.Metadata.TenantRef.ID != tenantID || value.Value.Metadata.UID != body.EnrollmentID || value.Value.Metadata.Name != body.WorkerName || value.Value.Spec.ProjectRef.ID != projectID || value.Value.Spec.WorkerID != body.WorkerID {
		return RemoteWorkerEnrollmentResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/metadata")
	}
	return value, nil
}
func (client *Client) GetAdminRemoteWorkerEnrollment(ctx context.Context, tenantID, projectID, enrollmentID, requestID string) (RemoteWorkerEnrollmentResult, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, enrollmentID, requestID); err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "GET", Path: "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/remote-worker-enrollments/" + enrollmentID, Headers: map[string]string{HeaderRequestID: requestID}})
	if err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
	if response.Status != 200 {
		return RemoteWorkerEnrollmentResult{}, client.problemError("adminGetRemoteWorkerEnrollment", response)
	}
	value, err := platform.DecodeRemoteWorkerEnrollmentResponseJSON(response.Body)
	if err != nil {
		return RemoteWorkerEnrollmentResult{}, &ClientError{Operation: "adminGetRemoteWorkerEnrollment", Status: response.Status, Cause: err}
	}
	if err := requireResourceVersion(response, value.Value.Metadata.ResourceVersion); err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
	if value.Value.Metadata.TenantRef.ID != tenantID || value.Value.Metadata.UID != enrollmentID || value.Value.Spec.ProjectRef.ID != projectID {
		return RemoteWorkerEnrollmentResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/metadata")
	}
	return value, nil
}
...
func (client *Client) RevokeAdminRemoteWorkerEnrollment(ctx context.Context, tenantID, projectID, enrollmentID, requestID, idempotencyKey string, body platform.RemoteWorkerEnrollmentRevokeRequest) (RemoteWorkerEnrollmentResult, error) {
	...
	bodyBytes, err := platform.EncodeRemoteWorkerEnrollmentRevokeRequestJSON(body)
	if err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "POST", Path: "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/remote-worker-enrollments/" + enrollmentID + ":revoke", Headers: map[string]string{HeaderRequestID: requestID, HeaderIdempotencyKey: idempotencyKey}, Body: bodyBytes})
	if err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
	if response.Status != 200 {
		return RemoteWorkerEnrollmentResult{}, client.problemError("adminRevokeRemoteWorkerEnrollment", response)
	}
	value, err := platform.DecodeRemoteWorkerEnrollmentResponseJSON(response.Body)
	if err != nil {
		return RemoteWorkerEnrollmentResult{}, &ClientError{Operation: "adminRevokeRemoteWorkerEnrollment", Status: response.Status, Cause: err}
	}
	if err := requireResourceVersion(response, value.Value.Metadata.ResourceVersion); err != nil {
		return RemoteWorkerEnrollmentResult{}, err
	}
...
func (client *Client) ClaimRemoteWorkerEnrollmentSecret(ctx context.Context, tenantID, projectID, enrollmentID, requestID, idempotencyKey string, body platform.RemoteWorkerEnrollmentSecretClaimRequest) (RemoteWorkerEnrollmentSecretResult, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, enrollmentID, requestID); err != nil {
		return RemoteWorkerEnrollmentSecretResult{}, err
	}
	if err := common.ValidateIdempotencyKey(idempotencyKey, "/Idempotency-Key"); err != nil {
		return RemoteWorkerEnrollmentSecretResult{}, err
	}
	bodyBytes, err := platform.EncodeRemoteWorkerEnrollmentSecretClaimRequestJSON(body)
	if err != nil {
		return RemoteWorkerEnrollmentSecretResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "POST", Path: "/v1/remote-worker-bootstrap/tenants/" + tenantID + "/projects/" + projectID + "/remote-worker-enrollments/" + enrollmentID + ":claimSecret", Headers: map[string]string{HeaderRequestID: requestID, HeaderIdempotencyKey: idempotencyKey}, Body: bodyBytes})
	if err != nil {
		return RemoteWorkerEnrollmentSecretResult{}, err
	}
	if response.Status != 200 {
		return RemoteWorkerEnrollmentSecretResult{}, client.problemError("remoteWorkerClaimEnrollmentSecret", response)
	}
	if response.Headers["Cache-Control"] != "no-store" || response.Headers["Pragma"] != "no-cache" {
		return RemoteWorkerEnrollmentSecretResult{}, common.ContractError("SECRET_CACHE_POLICY_MISMATCH", "/headers")
	}
	value, err := platform.DecodeRemoteWorkerEnrollmentSecretResponseJSON(response.Body)
	if err != nil {
		return RemoteWorkerEnrollmentSecretResult{}, &ClientError{Operation: "remoteWorkerClaimEnrollmentSecret", Status: response.Status, Cause: err}
	}
...
func (client *Client) ListAdminRemoteWorkerEnrollmentAuditEvents(ctx context.Context, tenantID, projectID, enrollmentID, requestID string, pageSize int, pageToken string) (AdminAuditEventPageResult, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, enrollmentID, requestID); err != nil {
		return AdminAuditEventPageResult{}, err
	}
	if pageSize != 0 && (pageSize < 1 || pageSize > 200) {
	...
	path := "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/remote-worker-enrollments/" + enrollmentID + "/audit-events"
	if encoded := query.Encode(); encoded != "" {
		path += "?" + encoded
	}
	response, err := client.roundTrip(ctx, Request{Method: "GET", Path: path, Headers: map[string]string{HeaderRequestID: requestID}})
	...
	if response.Status != 200 {
		return AdminAuditEventPageResult{}, client.problemError("adminListRemoteWorkerEnrollmentAuditEvents", response)
	}
	value, err := platform.DecodeAdminAuditEventPageResponseJSON(response.Body)
	if err != nil {
		return AdminAuditEventPageResult{}, &ClientError{Operation: "adminListRemoteWorkerEnrollmentAuditEvents", Status: response.Status, Cause: err}
	}
	for _, event := range value.Value.Events {
...
func (client *Client) ListRuntimeProfiles(ctx context.Context, tenantID, projectID, requestID string, pageSize int, pageToken string) (RuntimeProfileSummaryPageResult, error) {
	if pageSize == 0 {
		pageSize = 50
	}
	input, err := ValidateListRuntimeProfilesServerRequest(tenantID, projectID, requestID, pageSize, pageToken)
	if err != nil {
		return RuntimeProfileSummaryPageResult{}, err
	}
	query := url.Values{}
	query.Set("pageSize", strconv.Itoa(input.PageSize))
	if input.PageToken != "" {
	...
	requestPath := "/v1/tenants/" + tenantID + "/projects/" + projectID + "/runtime-profiles?" + query.Encode()
	...
	if response.Status != 200 {
		return RuntimeProfileSummaryPageResult{}, client.problemError("foundationListRuntimeProfiles", response)
	}
	value, err := platform.DecodeRuntimeProfileSummaryPageResponseJSON(response.Body)
	if err != nil {
		return RuntimeProfileSummaryPageResult{}, &ClientError{Operation: "foundationListRuntimeProfiles", Status: response.Status, Cause: err}
	}
	for _, profile := range value.Value.RuntimeProfiles {
		if profile.ProjectRef.ID != projectID {
			return RuntimeProfileSummaryPageResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/runtimeProfiles")
		}
	}
	return value, nil
}
func (client *Client) CreateSandbox(ctx context.Context, tenantID, projectID, requestID, idempotencyKey string, body platform.SandboxSessionCreateRequest) (SandboxSessionResult, error) {
	bodyBytes, err := platform.EncodeSandboxSessionCreateRequestJSON(body)
	if err != nil {
		return SandboxSessionResult{}, err
	}
	if _, err := ValidateCreateSandboxServerRequest(tenantID, projectID, requestID, idempotencyKey, bodyBytes); err != nil {
		return SandboxSessionResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "POST", Path: "/v1/tenants/" + tenantID + "/projects/" + projectID + "/sandbox-sessions", Headers: map[string]string{HeaderRequestID: requestID, HeaderIdempotencyKey: idempotencyKey}, Body: bodyBytes})
	if err != nil {
		return SandboxSessionResult{}, err
	}
	if response.Status != 202 {
		return SandboxSessionResult{}, client.problemError("foundationCreateSandbox", response)
	}
	value, err := platform.DecodeSandboxSessionResponseJSON(response.Body)
	if err != nil {
		return SandboxSessionResult{}, &ClientError{Operation: "foundationCreateSandbox", Status: response.Status, Cause: err}
	}
	got := value.Value
...
func (client *Client) ListAdminRuntimeProfiles(ctx context.Context, tenantID, projectID, requestID string, pageSize int, pageToken string) (RuntimeProfilePageResult, error) {
	...
	input, err := ValidateListAdminRuntimeProfilesServerRequest(tenantID, projectID, requestID, pageSize, pageToken)
	if err != nil {
		return RuntimeProfilePageResult{}, err
	}
	query := url.Values{}
	query.Set("pageSize", strconv.Itoa(input.PageSize))
	if input.PageToken != "" {
	...
	requestPath := "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/runtime-profiles?" + query.Encode()
	response, err := client.roundTrip(ctx, Request{Method: "GET", Path: requestPath, Headers: map[string]string{HeaderRequestID: requestID}})
	if err != nil {
		return RuntimeProfilePageResult{}, err
	}
	if response.Status != 200 {
		return RuntimeProfilePageResult{}, client.problemError("adminListRuntimeProfiles", response)
	}
	value, err := platform.DecodeRuntimeProfilePageResponseJSON(response.Body)
	if err != nil {
		return RuntimeProfilePageResult{}, &ClientError{Operation: "adminListRuntimeProfiles", Status: response.Status, Cause: err}
	}
	for _, profile := range value.Value.RuntimeProfiles {
		if profile.Metadata.TenantRef.ID != tenantID || profile.Spec.ProjectRef.ID != projectID {
			return RuntimeProfilePageResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/runtimeProfiles")
		}
	}
	return value, nil
}
func (client *Client) CreateAdminRuntimeProfile(ctx context.Context, tenantID, projectID, requestID, idempotencyKey string, body platform.RuntimeProfileCreateRequest) (RuntimeProfileResult, error) {
	bodyBytes, err := platform.EncodeRuntimeProfileCreateRequestJSON(body)
	if err != nil {
		return RuntimeProfileResult{}, err
	}
	input, err := ValidateCreateAdminRuntimeProfileServerRequest(tenantID, projectID, requestID, idempotencyKey, bodyBytes)
	if err != nil {
		return RuntimeProfileResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "POST", Path: "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/runtime-profiles", Headers: map[string]string{HeaderRequestID: requestID, HeaderIdempotencyKey: idempotencyKey}, Body: bodyBytes})
	if err != nil {
		return RuntimeProfileResult{}, err
	}
	if response.Status != 201 {
		return RuntimeProfileResult{}, client.problemError("adminCreateRuntimeProfile", response)
	}
	value, err := platform.DecodeRuntimeProfileResponseJSON(response.Body)
	if err != nil {
		return RuntimeProfileResult{}, &ClientError{Operation: "adminCreateRuntimeProfile", Status: response.Status, Cause: err}
	}
	if err := requireResourceVersion(response, value.Value.Metadata.ResourceVersion); err != nil {
		return RuntimeProfileResult{}, err
	}
	if value.Value.Metadata.TenantRef.ID != tenantID || value.Value.Metadata.Name != input.Body.ProfileName || value.Value.Spec.ProjectRef.ID != projectID || value.Value.Spec.ProfileID != input.Body.ProfileID || value.Value.Spec.Version != input.Body.Version {
		return RuntimeProfileResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/metadata")
	}
	return value, nil
}
func (client *Client) PublishAdminRuntimeProfile(ctx context.Context, tenantID, projectID, profileID string, version int64, requestID, idempotencyKey string, body platform.RuntimeProfileTransitionRequest) (RuntimeProfileResult, error) {
	return client.transitionAdminRuntimeProfile(ctx, "publish", "adminPublishRuntimeProfile", tenantID, projectID, profileID, version, requestID, idempotencyKey, body)
}
func (client *Client) DisableAdminRuntimeProfile(ctx context.Context, tenantID, projectID, profileID string, version int64, requestID, idempotencyKey string, body platform.RuntimeProfileTransitionRequest) (RuntimeProfileResult, error) {
	return client.transitionAdminRuntimeProfile(ctx, "disable", "adminDisableRuntimeProfile", tenantID, projectID, profileID, version, requestID, idempotencyKey, body)
}
func (client *Client) transitionAdminRuntimeProfile(ctx context.Context, action, operation, tenantID, projectID, profileID string, version int64, requestID, idempotencyKey string, body platform.RuntimeProfileTransitionRequest) (RuntimeProfileResult, error) {
	bodyBytes, err := platform.EncodeRuntimeProfileTransitionRequestJSON(body)
	if err != nil {
		return RuntimeProfileResult{}, err
	}
	if _, err := validateTransitionAdminRuntimeProfileServerRequest(tenantID, projectID, profileID, version, requestID, idempotencyKey, bodyBytes); err != nil {
		return RuntimeProfileResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "POST", Path: "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/runtime-profiles/" + profileID + "/versions/" + strconv.FormatInt(version, 10) + ":" + action, Headers: map[string]string{HeaderRequestID: requestID, HeaderIdempotencyKey: idempotencyKey}, Body: bodyBytes})
	if err != nil {
		return RuntimeProfileResult{}, err
	}
	if response.Status != 200 {
		return RuntimeProfileResult{}, client.problemError(operation, response)
	}
	value, err := platform.DecodeRuntimeProfileResponseJSON(response.Body)
	if err != nil {
		return RuntimeProfileResult{}, &ClientError{Operation: operation, Status: response.Status, Cause: err}
	}
	if err := requireResourceVersion(response, value.Value.Metadata.ResourceVersion); err != nil {
		return RuntimeProfileResult{}, err
	}
...
func (client *Client) GetAdminRuntimeProfile(ctx context.Context, tenantID, projectID, profileID string, version int64, requestID string) (RuntimeProfileResult, error) {
	if err := validateEnvironmentProfilePath(tenantID, projectID, profileID, version, requestID); err != nil {
		return RuntimeProfileResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "GET", Path: "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/runtime-profiles/" + profileID + "/versions/" + strconv.FormatInt(version, 10), Headers: map[string]string{HeaderRequestID: requestID}})
	if err != nil {
		return RuntimeProfileResult{}, err
	}
	if response.Status != 200 {
		return RuntimeProfileResult{}, client.problemError("adminGetRuntimeProfile", response)
	}
	value, err := platform.DecodeRuntimeProfileResponseJSON(response.Body)
	if err != nil {
		return RuntimeProfileResult{}, &ClientError{Operation: "adminGetRuntimeProfile", Status: response.Status, Cause: err}
	}
	if err := requireResourceVersion(response, value.Value.Metadata.ResourceVersion); err != nil {
		return RuntimeProfileResult{}, err
	}
...
func (client *Client) CleanupAdminWorkspaceSnapshot(ctx context.Context, tenantID, projectID, snapshotID, requestID, idempotencyKey string, body platform.WorkspaceSnapshotCleanupRequest) (WorkspaceSnapshotResult, error) {
	bodyBytes, err := platform.EncodeWorkspaceSnapshotCleanupRequestJSON(body)
	if err != nil {
		return WorkspaceSnapshotResult{}, err
	}
	input, err := ValidateCleanupAdminWorkspaceSnapshotServerRequest(tenantID, projectID, snapshotID, requestID, idempotencyKey, bodyBytes)
	if err != nil {
		return WorkspaceSnapshotResult{}, err
	}
	response, err := client.roundTrip(ctx, Request{Method: "POST", Path: "/v1/admin/tenants/" + tenantID + "/projects/" + projectID + "/workspace-snapshots/" + snapshotID + ":cleanup", Headers: map[string]string{HeaderRequestID: requestID, HeaderIdempotencyKey: idempotencyKey}, Body: bodyBytes})
	if err != nil {
		return WorkspaceSnapshotResult{}, err
	}
	if response.Status != 202 {
		return WorkspaceSnapshotResult{}, client.problemError("adminCleanupWorkspaceSnapshot", response)
	}
	value, err := platform.DecodeWorkspaceSnapshotResponseJSON(response.Body)
	if err != nil {
		return WorkspaceSnapshotResult{}, err
	...
	if value.Value.Metadata.TenantRef.ID != tenantID || value.Value.Spec.ProjectRef.ID != projectID || value.Value.Metadata.UID != snapshotID || value.Value.Spec.SourceWorkspaceID != input.Body.ConfirmedSourceWorkspaceID {
		return WorkspaceSnapshotResult{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/metadata")
	}
	return value, nil
}
...
type ListAdminRemoteWorkerEnrollmentsServerInput struct {
	TenantID  string
	ProjectID string
	RequestID string
	PageSize  int
...
func ValidateListAdminRemoteWorkerEnrollmentsServerRequest(tenantID, projectID, requestID string, pageSize int, pageToken string) (ListAdminRemoteWorkerEnrollmentsServerInput, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, "", requestID); err != nil {
		return ListAdminRemoteWorkerEnrollmentsServerInput{}, err
	}
	if pageSize < 1 || pageSize > 200 {
		return ListAdminRemoteWorkerEnrollmentsServerInput{}, common.ContractError("INVALID_PAGE_SIZE", "/pageSize")
	}
	if pageToken != "" {
		if err := common.ValidatePageToken(pageToken, "/pageToken"); err != nil {
			return ListAdminRemoteWorkerEnrollmentsServerInput{}, err
	...
	return ListAdminRemoteWorkerEnrollmentsServerInput{TenantID: tenantID, ProjectID: projectID, RequestID: requestID, PageSize: pageSize, PageToken: pageToken}, nil
}

type CreateAdminRemoteWorkerEnrollmentServerInput struct {
	TenantID       string
	ProjectID      string
	RequestID      string
	IdempotencyKey string
	Body           platform.RemoteWorkerEnrollmentCreateRequest
}

func ValidateCreateAdminRemoteWorkerEnrollmentServerRequest(tenantID, projectID, requestID, idempotencyKey string, body []byte) (CreateAdminRemoteWorkerEnrollmentServerInput, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, "", requestID); err != nil {
		return CreateAdminRemoteWorkerEnrollmentServerInput{}, err
	}
	if err := common.ValidateIdempotencyKey(idempotencyKey, "/Idempotency-Key"); err != nil {
		return CreateAdminRemoteWorkerEnrollmentServerInput{}, err
	}
	value, err := platform.DecodeRemoteWorkerEnrollmentCreateRequestJSON(body)
	if err != nil {
		return CreateAdminRemoteWorkerEnrollmentServerInput{}, err
	}
	return CreateAdminRemoteWorkerEnrollmentServerInput{TenantID: tenantID, ProjectID: projectID, RequestID: requestID, IdempotencyKey: idempotencyKey, Body: value}, nil
}

type GetAdminRemoteWorkerEnrollmentServerInput struct {
	TenantID     string
	ProjectID    string
	EnrollmentID string
	RequestID    string
...
func ValidateGetAdminRemoteWorkerEnrollmentServerRequest(tenantID, projectID, enrollmentID, requestID string) (GetAdminRemoteWorkerEnrollmentServerInput, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, enrollmentID, requestID); err != nil {
		return GetAdminRemoteWorkerEnrollmentServerInput{}, err
	}
...
type RevokeAdminRemoteWorkerEnrollmentServerInput struct {
	...
	RequestID      string
	IdempotencyKey string
	Body           platform.RemoteWorkerEnrollmentRevokeRequest
}

func ValidateRevokeAdminRemoteWorkerEnrollmentServerRequest(tenantID, projectID, enrollmentID, requestID, idempotencyKey string, body []byte) (RevokeAdminRemoteWorkerEnrollmentServerInput, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, enrollmentID, requestID); err != nil {
		return RevokeAdminRemoteWorkerEnrollmentServerInput{}, err
	}
	if err := common.ValidateIdempotencyKey(idempotencyKey, "/Idempotency-Key"); err != nil {
		return RevokeAdminRemoteWorkerEnrollmentServerInput{}, err
	}
	value, err := platform.DecodeRemoteWorkerEnrollmentRevokeRequestJSON(body)
	if err != nil {
		return RevokeAdminRemoteWorkerEnrollmentServerInput{}, err
	}
	return RevokeAdminRemoteWorkerEnrollmentServerInput{TenantID: tenantID, ProjectID: projectID, EnrollmentID: enrollmentID, RequestID: requestID, IdempotencyKey: idempotencyKey, Body: value}, nil
}

type ClaimRemoteWorkerEnrollmentSecretServerInput struct {
	TenantID       string
	ProjectID      string
	EnrollmentID   string
	RequestID      string
	IdempotencyKey string
	Body           platform.RemoteWorkerEnrollmentSecretClaimRequest
}

func ValidateClaimRemoteWorkerEnrollmentSecretServerRequest(tenantID, projectID, enrollmentID, requestID, idempotencyKey string, body []byte) (ClaimRemoteWorkerEnrollmentSecretServerInput, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, enrollmentID, requestID); err != nil {
	...
	value, err := platform.DecodeRemoteWorkerEnrollmentSecretClaimRequestJSON(body)
	if err != nil {
		return ClaimRemoteWorkerEnrollmentSecretServerInput{}, err
	}
...
func ValidateListAdminRemoteWorkerEnrollmentAuditEventsServerRequest(tenantID, projectID, enrollmentID, requestID string, pageSize int, pageToken string) (ListAdminRemoteWorkerEnrollmentAuditEventsServerInput, error) {
	if err := validateRemoteWorkerEnrollmentPath(tenantID, projectID, enrollmentID, requestID); err != nil {
		return ListAdminRemoteWorkerEnrollmentAuditEventsServerInput{}, err
	}
	if pageSize < 1 || pageSize > 200 {
		return ListAdminRemoteWorkerEnrollmentAuditEventsServerInput{}, common.ContractError("INVALID_PAGE_SIZE", "/pageSize")
	}
	if pageToken != "" {
		if err := common.ValidatePageToken(pageToken, "/pageToken"); err != nil {
			return ListAdminRemoteWorkerEnrollmentAuditEventsServerInput{}, err
...
func validateRemoteWorkerEnrollmentPath(tenantID, projectID, enrollmentID, requestID string) error {
	...
	if enrollmentID != "" {
		return common.ValidateIdentifier(enrollmentID, "/remoteWorkerEnrollmentId")
	}
	return nil
}
...
type CreateAdminRuntimeProfileServerInput struct {
	TenantID       string
	ProjectID      string
	RequestID      string
	IdempotencyKey string
...
func ValidateCreateAdminRuntimeProfileServerRequest(tenantID, projectID, requestID, idempotencyKey string, body []byte) (CreateAdminRuntimeProfileServerInput, error) {
	if err := validateEnvironmentProfilePath(tenantID, projectID, "", 0, requestID); err != nil {
		return CreateAdminRuntimeProfileServerInput{}, err
	}
	if err := common.ValidateIdempotencyKey(idempotencyKey, "/Idempotency-Key"); err != nil {
		return CreateAdminRuntimeProfileServerInput{}, err
	}
	value, err := platform.DecodeRuntimeProfileCreateRequestJSON(body)
	if err != nil {
		return CreateAdminRuntimeProfileServerInput{}, err
	}
	return CreateAdminRuntimeProfileServerInput{TenantID: tenantID, ProjectID: projectID, RequestID: requestID, IdempotencyKey: idempotencyKey, Body: value}, nil
}

type TransitionAdminRuntimeProfileServerInput struct {
	TenantID       string
	ProjectID      string
	ProfileID      string
	Version        int64
	RequestID      string
	IdempotencyKey string
	Body           platform.RuntimeProfileTransitionRequest
}

func ValidatePublishAdminRuntimeProfileServerRequest(tenantID, projectID, profileID string, version int64, requestID, idempotencyKey string, body []byte) (TransitionAdminRuntimeProfileServerInput, error) {
	return validateTransitionAdminRuntimeProfileServerRequest(tenantID, projectID, profileID, version, requestID, idempotencyKey, body)
}
func ValidateDisableAdminRuntimeProfileServerRequest(tenantID, projectID, profileID string, version int64, requestID, idempotencyKey string, body []byte) (TransitionAdminRuntimeProfileServerInput, error) {
	return validateTransitionAdminRuntimeProfileServerRequest(tenantID, projectID, profileID, version, requestID, idempotencyKey, body)
}
func validateTransitionAdminRuntimeProfileServerRequest(tenantID, projectID, profileID string, version int64, requestID, idempotencyKey string, body []byte) (TransitionAdminRuntimeProfileServerInput, error) {
	if err := validateEnvironmentProfilePath(tenantID, projectID, profileID, version, requestID); err != nil {
		return TransitionAdminRuntimeProfileServerInput{}, err
	}
	if err := common.ValidateIdempotencyKey(idempotencyKey, "/Idempotency-Key"); err != nil {
		return TransitionAdminRuntimeProfileServerInput{}, err
	}
	value, err := platform.DecodeRuntimeProfileTransitionRequestJSON(body)
	if err != nil {
		return TransitionAdminRuntimeProfileServerInput{}, err
	}
	return TransitionAdminRuntimeProfileServerInput{TenantID: tenantID, ProjectID: projectID, ProfileID: profileID, Version: version, RequestID: requestID, IdempotencyKey: idempotencyKey, Body: value}, nil
}

type ListRuntimeProfilesServerInput struct {
	TenantID  string
	ProjectID string
	RequestID string
	PageSize  int
...
func ValidateListAdminRuntimeProfilesServerRequest(tenantID, projectID, requestID string, pageSize int, pageToken string) (ListRuntimeProfilesServerInput, error) {
	return validateListRuntimeProfilesServerRequest(tenantID, projectID, requestID, pageSize, pageToken)
}
func ValidateListRuntimeProfilesServerRequest(tenantID, projectID, requestID string, pageSize int, pageToken string) (ListRuntimeProfilesServerInput, error) {
	return validateListRuntimeProfilesServerRequest(tenantID, projectID, requestID, pageSize, pageToken)
}
func validateListRuntimeProfilesServerRequest(tenantID, projectID, requestID string, pageSize int, pageToken string) (ListRuntimeProfilesServerInput, error) {
	if err := validateEnvironmentProfilePath(tenantID, projectID, "", 0, requestID); err != nil {
		return ListRuntimeProfilesServerInput{}, err
	}
	if pageSize < 1 || pageSize > 200 {
		return ListRuntimeProfilesServerInput{}, common.ContractError("INVALID_PAGE_SIZE", "/pageSize")
	}
	if pageToken != "" {
		if err := common.ValidatePageToken(pageToken, "/pageToken"); err != nil {
			return ListRuntimeProfilesServerInput{}, err
	...
	return ListRuntimeProfilesServerInput{TenantID: tenantID, ProjectID: projectID, RequestID: requestID, PageSize: pageSize, PageToken: pageToken}, nil
}

type GetAdminRuntimeProfileServerInput struct {
	TenantID  string
	ProjectID string
	ProfileID string
	Version   int64
...
func ValidateGetAdminRuntimeProfileServerRequest(tenantID, projectID, profileID string, version int64, requestID string) (GetAdminRuntimeProfileServerInput, error) {
	if err := validateEnvironmentProfilePath(tenantID, projectID, profileID, version, requestID); err != nil {
...
type CleanupAdminWorkspaceSnapshotServerInput struct {
	TenantID       string
	ProjectID      string
	SnapshotID     string
	RequestID      string
	IdempotencyKey string
	Body           platform.WorkspaceSnapshotCleanupRequest
}

func ValidateCleanupAdminWorkspaceSnapshotServerRequest(tenantID, projectID, snapshotID, requestID, idempotencyKey string, body []byte) (CleanupAdminWorkspaceSnapshotServerInput, error) {
	base, err := ValidateGetAdminWorkspaceSnapshotServerRequest(tenantID, projectID, snapshotID, requestID)
	if err != nil {
		return CleanupAdminWorkspaceSnapshotServerInput{}, err
	}
	if err := common.ValidateIdempotencyKey(idempotencyKey, "/Idempotency-Key"); err != nil {
		return CleanupAdminWorkspaceSnapshotServerInput{}, err
	}
	value, err := platform.DecodeWorkspaceSnapshotCleanupRequestJSON(body)
	if err != nil {
		return CleanupAdminWorkspaceSnapshotServerInput{}, err
	}
	if value.ConfirmedSnapshotID != snapshotID {
		return CleanupAdminWorkspaceSnapshotServerInput{}, common.ContractError("PATH_BODY_AUTHORITY_MISMATCH", "/confirmedSnapshotId")
	}
	return CleanupAdminWorkspaceSnapshotServerInput{TenantID: base.TenantID, ProjectID: base.ProjectID, SnapshotID: base.SnapshotID, RequestID: base.RequestID, IdempotencyKey: idempotencyKey, Body: value}, nil
}
...
type CreateSandboxServerInput struct {
	...
	RequestID      string
	IdempotencyKey string
	Body           platform.SandboxSessionCreateRequest
}

func ValidateCreateSandboxServerRequest(tenantID, projectID, requestID, idempotencyKey string, body []byte) (CreateSandboxServerInput, error) {
	if err := validateEnvironmentProfilePath(tenantID, projectID, "", 0, requestID); err != nil {
	...
	value, err := platform.DecodeSandboxSessionCreateRequestJSON(body)
	if err != nil {
		return CreateSandboxServerInput{}, err
	}
