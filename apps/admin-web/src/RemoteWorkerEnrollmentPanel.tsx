...
const nodeHealthKeys: Readonly<Record<NonNullable<RemoteWorkerEnrollment["spec"]["node"]>["healthState"], MessageKey>> = {
...
const nodeStateKeys: Readonly<Record<NonNullable<RemoteWorkerEnrollment["spec"]["node"]>["observedState"], MessageKey>> = {
    ...
    <section className="resource-list">
      ...
      {selected ? (
        <section className="panel overview-panel">
          ...
          {selected.spec.node ? <section className="activity-block"><div className="activity-heading"><h3>{t("remoteWorkerEnrollment.node.title")}</h3><span className={`phase ${selected.spec.node.healthState === "online" ? "success" : selected.spec.node.healthState === "degraded" ? "running" : "danger"}`}><i />{t(nodeHealthKeys[selected.spec.node.healthState])}</span></div><dl className="detail-grid"><div><dt>{t("remoteWorkerEnrollment.node.state")}</dt><dd>{t(nodeStateKeys[selected.spec.node.observedState])}</dd></div><div><dt>{t("remoteWorkerEnrollment.node.generation")}</dt><dd className="mono">{number(selected.spec.node.observedGeneration)} / {number(selected.spec.node.generation)}</dd></div><div><dt>{t("remoteWorkerEnrollment.node.resourceVersion")}</dt><dd className="mono">{selected.spec.node.resourceVersion}</dd></div><div><dt>{t("remoteWorkerEnrollment.node.workerVersion")}</dt><dd className="mono">{selected.spec.node.workerVersion}</dd></div><div><dt>{t("remoteWorkerEnrollment.node.platform")}</dt><dd className="mono">{selected.spec.node.os} · {selected.spec.node.architecture} · {selected.spec.node.kernelVersion}</dd></div><div><dt>{t("remoteWorkerEnrollment.node.capabilities")}</dt><dd className="mono">{selected.spec.node.capabilities.join(", ")}</dd></div><div><dt>{t("remoteWorkerEnrollment.node.capacity")}</dt><dd>{number(selected.spec.node.capacity.cpuMillis)} mCPU · {number(selected.spec.node.capacity.memoryBytes)} B · {number(selected.spec.node.capacity.diskBytes)} B</dd></div><div><dt>{t("remoteWorkerEnrollment.node.firstConnected")}</dt><dd>{dateTime(selected.spec.node.firstConnectedAt)}</dd></div><div><dt>{t("remoteWorkerEnrollment.node.lastHeartbeat")}</dt><dd>{dateTime(selected.spec.node.lastHeartbeatAt)}</dd></div><div><dt>{t("remoteWorkerEnrollment.node.heartbeatExpires")}</dt><dd>{dateTime(selected.spec.node.heartbeatExpiresAt)}</dd></div></dl><p className="cluster-boundary">{t("remoteWorkerEnrollment.node.boundary")}</p></section> : null}
