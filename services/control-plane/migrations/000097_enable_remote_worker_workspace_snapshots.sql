-- Enable portable Workspace snapshot claims and restores on RemoteWorker targets.
...
AS $$
    SELECT pg_catalog.cardinality(p_values) BETWEEN 1 AND 16
       AND p_values <@ ARRAY[
           ...
           'network-internal-deny','preview','pty','ssh','workspace-snapshot','workspace-volume'
       ]::text[]
       AND p_values = ARRAY(
...
-- Keep the shared snapshot reaper usable when its OUT-column names overlap table columns.
CREATE OR REPLACE FUNCTION cloud_agents.reap_foundation_workspace_snapshot_claim_v1(p_subject text, p_audit text)
...
BEGIN
    ...
    UPDATE cloud_agents.operation_attempts AS attempt SET state = CASE WHEN candidate.delivery_attempts < 8 THEN 'unknown' ELSE 'failed' END,
    ...
    WHERE attempt.tenant_id = candidate.tenant_id AND attempt.operation_id = candidate.operation_id
      AND attempt.operation_generation = candidate.operation_generation AND attempt.attempt_number = candidate.delivery_attempts AND attempt.state = 'claimed';
    IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE = '40001', MESSAGE = 'workspace snapshot attempt authority drifted'; END IF;
    UPDATE cloud_agents.workspace_snapshots AS snapshot SET status = CASE WHEN candidate.delivery_attempts < 8 THEN 'unknown' ELSE 'failed' END,
    ...
    WHERE snapshot.tenant_id = candidate.tenant_id AND snapshot.operation_id = candidate.operation_id AND snapshot.snapshot_uid = candidate.aggregate_id;
    IF candidate.delivery_attempts < 8 THEN
      outbox_state := 'pending';
      UPDATE cloud_agents.outbox_events AS event SET state = 'pending', claim_holder_id = NULL, claim_incarnation = NULL,
      ...
      WHERE operation.tenant_id = candidate.tenant_id AND operation.operation_id = candidate.operation_id AND operation.operation_generation = candidate.operation_generation;
    ELSE
      ...
      UPDATE cloud_agents.outbox_events AS event SET state = 'dead_letter', claim_holder_id = NULL, claim_incarnation = NULL,
      ...
      WHERE event.tenant_id = candidate.tenant_id AND event.event_id = candidate.event_id;
      UPDATE cloud_agents.platform_operations AS operation SET state = 'failed', cleanup_phase = 'blocked', terminal_error_code = error_code,
        updated_at = reaped_at, terminal_at = reaped_at
      WHERE operation.tenant_id = candidate.tenant_id AND operation.operation_id = candidate.operation_id AND operation.operation_generation = candidate.operation_generation;
      UPDATE cloud_agents.operation_finalizers AS finalizer SET state = 'dead_letter', delivery_attempts = candidate.delivery_attempts,
        stable_error_code = error_code, updated_at = reaped_at, terminal_at = reaped_at
      WHERE finalizer.tenant_id = candidate.tenant_id AND finalizer.operation_id = candidate.operation_id AND finalizer.operation_generation = candidate.operation_generation;
      UPDATE cloud_agents.idempotency_records AS record SET state = 'failed', stable_error_code = error_code, updated_at = reaped_at, terminal_at = reaped_at
      WHERE record.tenant_id = candidate.tenant_id AND record.operation_id = candidate.operation_id AND record.operation_generation = candidate.operation_generation;
...
BEGIN
    ...
    WHERE sandbox.tenant_id = tenant AND sandbox.project_uid = p_project AND sandbox.sandbox_uid = p_sandbox
      ...
      AND target.target_kind IN ('docker', 'kubernetes', 'remote-worker') AND target.observed_phase = 'ready' AND target.scheduling_state = 'active'
...
CREATE OR REPLACE FUNCTION cloud_agents.claim_foundation_workspace_snapshot_remote_worker_v1(
    p_holder text, p_incarnation text, p_token text, p_lease_seconds integer, p_subject text, p_audit text
)
RETURNS TABLE (
    tenant_id text, event_id text, delivery_attempts integer, claim_expires_at timestamptz,
...
BEGIN
    ...
    WHERE event.profile_id = 'foundationWorkspaceSnapshotCleanup/v1alpha1' AND event.profile_digest = 'sha256:6157975f3a02c036472fca30bb309fcdee757c2cc99d04a0f053bb7cfd064f69'
      ...
      AND snapshot.physical_snapshot_uid IS NOT NULL AND target.target_kind IN ('docker', 'kubernetes', 'remote-worker')
    ORDER BY event.created_at, event.tenant_id, event.event_id FOR UPDATE OF event SKIP LOCKED LIMIT 1;
    IF NOT FOUND THEN RETURN; END IF;
    PERFORM pg_catalog.set_config('cloud_agents.tenant_id', candidate.tenant_id, true);
    UPDATE cloud_agents.outbox_events AS event SET state = 'claimed', delivery_attempts = event.delivery_attempts + 1,
