...
BEGIN
    ...
    -- A new Membership must not reactivate an otherwise ineligible historical
    -- RoleBinding. A binding already covered by another eligible Membership is
    -- not stale and does not block a narrower admission.
    IF EXISTS (
        SELECT 1
        ...
        WHERE binding.tenant_id = p_tenant_id
            ...
            AND (binding.expires_at IS NULL OR mutation_at < binding.expires_at)
            AND (
                ...
                OR (
                    ...
                    AND (
                        (
                            binding.scope_level = 'organization'
                            AND binding.scope_organization_uid = p_scope_uid
                        )
                        OR (
                            binding.scope_level = 'project'
                            AND EXISTS (
                                SELECT 1
                                FROM cloud_agents.projects AS binding_project
                                WHERE binding_project.tenant_id = binding.tenant_id
                                    AND binding_project.project_uid = binding.scope_project_uid
                                    AND binding_project.organization_uid = p_scope_uid
                            )
                        )
                    )
                )
                OR (
                    ...
                    AND binding.scope_level = 'project'
                    AND binding.scope_project_uid = p_scope_uid
                )
            )
            AND NOT EXISTS (
                SELECT 1
                FROM cloud_agents.memberships AS existing_membership
                WHERE existing_membership.tenant_id = binding.tenant_id
                    AND existing_membership.subject_kind = binding.subject_kind
                    AND existing_membership.subject_issuer = binding.subject_issuer
                    AND existing_membership.subject_value = binding.subject_value
                    AND existing_membership.subject_digest = binding.subject_digest
                    AND existing_membership.state = 'active'
                    AND (
                        existing_membership.expires_at IS NULL
                        OR mutation_at < existing_membership.expires_at
                    )
                    AND (
                        existing_membership.scope_level = 'tenant'
                        OR (
                            existing_membership.scope_level = 'organization'
                            AND (
                                (
                                    binding.scope_level = 'organization'
                                    AND binding.scope_organization_uid =
                                        existing_membership.scope_organization_uid
                                )
                                OR (
                                    binding.scope_level = 'project'
                                    AND EXISTS (
                        ...
                        OR (
                            existing_membership.scope_level = 'project'
                            AND binding.scope_level = 'project'
                            AND existing_membership.scope_project_uid = binding.scope_project_uid
                        )
                    )
            )
    ) THEN
        RAISE EXCEPTION USING
            ERRCODE = '40001',
            MESSAGE = 'membership creation would reactivate a stale role binding';
    END IF;
