...
ALTER TABLE cloud_agents.deployment_targets
    ADD CONSTRAINT deployment_targets_kind CHECK (target_kind IN ('docker', 'kubernetes'));

CREATE FUNCTION cloud_agents.register_deployment_target_v2(
...
BEGIN
    ...
    IF p_tenant_id IS DISTINCT FROM cloud_agents.require_tenant_id()
        ...
        OR p_target_kind NOT IN ('docker', 'kubernetes')
