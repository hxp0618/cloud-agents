...
func TestRunSessionList(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodGet || request.URL.Path != "/v1/tenants/tenant-alpha/projects/project-alpha/sessions" || request.URL.Query().Get("pageSize") != "1" || request.URL.Query().Get("pageToken") != "session-page-token-1" {
			t.Fatalf("request = %s %s", request.Method, request.URL.String())
		}
		_, _ = writer.Write([]byte(`{"apiVersion":"managed-agent.cloud-agents.dev/v1alpha1","kind":"SessionPage","sessions":[],"nextPageToken":"session-page-token-2"}`))
	}))
	defer server.Close()
	var stdout bytes.Buffer
	err := run([]string{
	...
	if err != nil || !strings.Contains(stdout.String(), `"kind":"SessionPage"`) {
...
func TestRunTurnList(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_, _ = writer.Write([]byte(`{"apiVersion":"managed-agent.cloud-agents.dev/v1alpha1","kind":"TurnPage","turns":[],"nextPageToken":"turn-page-token-2"}`))
	}))
	defer server.Close()
	var stdout bytes.Buffer
	err := run([]string{
	...
	if err != nil || !strings.Contains(stdout.String(), `"kind":"TurnPage"`) {
		t.Fatalf("output/error = %q / %v", stdout.String(), err)
	}
}
...
func TestRunRoleList(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_, _ = writer.Write([]byte(`{"apiVersion":"platform.cloud-agents.dev/v1alpha1","kind":"RolePage","roles":[],"nextPageToken":"role-page-token-2"}`))
	}))
	defer server.Close()
	var stdout bytes.Buffer
	err := run([]string{
	...
	if err != nil || !strings.Contains(stdout.String(), `"kind":"RolePage"`) {
		t.Fatalf("output/error = %q / %v", stdout.String(), err)
	}
}
