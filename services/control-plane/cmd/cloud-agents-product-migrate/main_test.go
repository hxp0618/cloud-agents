...
func TestParseProductMigrationConfigUsesDatabaseEnvironment(t *testing.T) {
	...
	if config.databaseURL == "" || config.repositoryRoot != "/srv/cloud-agents" || config.selector != "product-000016" {
		t.Fatalf("config = %+v", config)
	}
}

func TestParseProductMigrationConfigRejectsSelectorDrift(t *testing.T) {
	if _, err := parseProductMigrationConfig([]string{"--selector", "product-000015", "--database-url", "postgres://migration@db/cloud_agents"}, nil); err == nil {
