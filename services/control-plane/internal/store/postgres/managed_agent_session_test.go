...
func TestScanManagedAgentSessionBuildsDetachedSnapshot(t *testing.T) {
	...
	err := scanManagedAgentSession(rowValues("session-alpha", "codex", nil, nil, nil, nil, nil, nil, nil, "active", int64(2), now, now), internalmanagedagent.Scope{TenantID: "tenant-alpha", ProjectID: "project-alpha"}, &snapshot)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = scanManagedAgentSession(rowValues("session-alpha", "codex", &leaseID, &generation, nil, nil, nil, &profileID, &profileVersion, "active", int64(2), now, now), internalmanagedagent.Scope{TenantID: "tenant-alpha", ProjectID: "project-alpha"}, &snapshot)
	...
	if err := scanManagedAgentSession(rowValues("session-alpha", "codex", &leaseID, nil, nil, nil, nil, nil, nil, "active", int64(2), now, now), internalmanagedagent.Scope{TenantID: "tenant-alpha", ProjectID: "project-alpha"}, &snapshot); !errors.Is(err, ErrCoordinationResultDrift) {
		t.Fatalf("partial environment binding error = %v", err)
	}
	if err := scanManagedAgentSession(rowValues("session-alpha", "codex", &leaseID, &generation, nil, nil, nil, &profileID, nil, "active", int64(2), now, now), internalmanagedagent.Scope{TenantID: "tenant-alpha", ProjectID: "project-alpha"}, &snapshot); !errors.Is(err, ErrCoordinationResultDrift) {
		t.Fatalf("partial profile binding error = %v", err)
	}
	workspaceID, sandboxID, sandboxGeneration := "workspace-alpha", "sandbox-alpha", int64(9)
	err = scanManagedAgentSession(rowValues("session-alpha", "codex", nil, nil, &workspaceID, &sandboxID, &sandboxGeneration, &profileID, &profileVersion, "active", int64(2), now, now), internalmanagedagent.Scope{TenantID: "tenant-alpha", ProjectID: "project-alpha"}, &snapshot)
	if err != nil || snapshot.WorkspaceID != workspaceID || snapshot.SandboxID != sandboxID || snapshot.SandboxGeneration != 9 || snapshot.EnvironmentLeaseID != "" {
		t.Fatalf("foundation snapshot = %#v / %v", snapshot, err)
	}
}
