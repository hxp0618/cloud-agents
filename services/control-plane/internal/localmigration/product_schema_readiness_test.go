...
func TestCheckProductSchemaReadiness(t *testing.T) {
	current := productRunnerBindingSelector("000054")
	tests := []struct {
		name    string
		row     readinessRow
		wantErr bool
	}{
		{name: "current", row: readinessRow{count: 54, first: "000001", last: "000054", bundleDigest: current.schemaBundleDigest}},
		...
		{name: "ahead", row: readinessRow{count: 55, first: "000001", last: "000055", bundleDigest: current.schemaBundleDigest}, wantErr: true},
		{name: "wrong bundle", row: readinessRow{count: 54, first: "000001", last: "000054", bundleDigest: productRunnerBindingSelector("000053").schemaBundleDigest}, wantErr: true},
