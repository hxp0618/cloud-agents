...
func TestCheckProductSchemaReadiness(t *testing.T) {
	current := productRunnerBindingSelector("000050")
	tests := []struct {
		name    string
		row     readinessRow
		wantErr bool
	}{
		{name: "current", row: readinessRow{count: 50, first: "000001", last: "000050", bundleDigest: current.schemaBundleDigest}},
		{name: "previous head", row: readinessRow{count: 49, first: "000001", last: "000049", bundleDigest: productRunnerBindingSelector("000049").schemaBundleDigest}, wantErr: true},
		...
		{name: "ahead", row: readinessRow{count: 51, first: "000001", last: "000051", bundleDigest: current.schemaBundleDigest}, wantErr: true},
		{name: "wrong bundle", row: readinessRow{count: 50, first: "000001", last: "000050", bundleDigest: productRunnerBindingSelector("000049").schemaBundleDigest}, wantErr: true},
