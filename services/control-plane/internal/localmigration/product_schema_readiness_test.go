...
func TestCheckProductSchemaReadiness(t *testing.T) {
	current := productRunnerBindingSelector("000039")
	tests := []struct {
		name    string
		row     readinessRow
		wantErr bool
	}{
		{name: "current", row: readinessRow{count: 39, first: "000001", last: "000039", bundleDigest: current.schemaBundleDigest}},
		...
		{name: "stale current", row: readinessRow{count: 38, first: "000001", last: "000038", bundleDigest: productRunnerBindingSelector("000038").schemaBundleDigest}, wantErr: true},
		{name: "ahead", row: readinessRow{count: 40, first: "000001", last: "000040", bundleDigest: current.schemaBundleDigest}, wantErr: true},
		{name: "wrong bundle", row: readinessRow{count: 39, first: "000001", last: "000039", bundleDigest: productRunnerBindingSelector("000038").schemaBundleDigest}, wantErr: true},
