...
func TestLoadAndVerifyLatestIndependentProductManifest(t *testing.T) {
	config := testConfig(t)
	config.ManifestSelector = "product-000047"
	config.ManifestPath = "services/control-plane/migrations/product/000047/manifest.json"
	bundle, err := loadAndVerify(config)
	if err != nil {
		t.Fatal(err)
	}
	if bundle.manifest.SchemaBundle.SchemaHead != "000047" || len(bundle.manifest.SchemaBundle.Migrations) != 47 {
...
func TestSupportedManifestLengthsAreVersioned(t *testing.T) {
	...
	if length, ok := supportedManifestLength("000048"); ok || length != 0 {
