...
func TestLoadAndVerifyLatestIndependentProductManifest(t *testing.T) {
	config := testConfig(t)
	config.ManifestSelector = "product-000055"
	config.ManifestPath = "services/control-plane/migrations/product/000055/manifest.json"
	bundle, err := loadAndVerify(config)
	if err != nil {
		t.Fatal(err)
	}
	if bundle.manifest.SchemaBundle.SchemaHead != "000055" || len(bundle.manifest.SchemaBundle.Migrations) != 55 {
...
func TestSupportedManifestLengthsAreVersioned(t *testing.T) {
	...
	if length, ok := supportedManifestLength("000056"); ok || length != 0 {
