...
type RecoveryBindingTransitionResult struct {
	outcome           evidencefs.AdmissionTransitionOutcome
	next              *RecoveryBoundPermit
	candidateDigest   [32]byte
	candidateSequence uint64
	candidateRevision uint64
	previousRevision  uint64
...
func (r RecoveryBindingTransitionResult) Outcome() evidencefs.AdmissionTransitionOutcome {
	return r.outcome
}
func (r RecoveryBindingTransitionResult) Next() *RecoveryBoundPermit { return r.next }
func (r RecoveryBindingTransitionResult) CandidateKind() string      { return "decision_recovery_binding" }
func (r RecoveryBindingTransitionResult) CandidateDigest() [32]byte  { return r.candidateDigest }
func (r RecoveryBindingTransitionResult) CandidateSequence() uint64  { return r.candidateSequence }
func (r RecoveryBindingTransitionResult) CandidateRevision() uint64  { return r.candidateRevision }
func (r RecoveryBindingTransitionResult) PreviousRevision() uint64   { return r.previousRevision }

type RecoveryBoundPermit struct {
	self                *RecoveryBoundPermit
	prior               *RecoveryPublishedPermit
	plan                *VerifiedAdmissionPlan
	candidateBinding    *verifiedEvidenceRunBinding
	inventory           *evidencefs.AdmissionInventory
	mutation            *evidencefs.AdmissionMutationToken
	...
	recoveryDigest      Digest
	recoverySize        uint64
	binding             *recoveryBoundPermitBinding
	consumed            *atomic.Bool
}

type recoveryBoundPermitBinding struct {
...
var recoveryBoundPermitRegistry sync.Map
...
func (p *RecoveryPublishedPermit) BindDecisionRecovery(ctx context.Context, candidate OwnedCurrentCandidate) (RecoveryBindingTransitionResult, error) {
	pre := RecoveryBindingTransitionResult{outcome: evidencefs.AdmissionTransitionPreMutationFailure, candidateDigest: digestRaw(candidate.decisionRecoveryArtifact.digest), candidateSequence: 4}
	if p == nil || p.inventory == nil || !validRecoveryPublishedPermit(p, p.inventory, candidate) {
		return pre, fail(CodeEvidenceRecoveryRequired, "admission-recovery-bind", "recovery publication permit is unavailable", nil)
	}
	pre.previousRevision = p.revision
	pre.candidateRevision = p.revision + 1
	if !p.consumed.CompareAndSwap(false, true) {
		return pre, fail(CodeEvidenceRecoveryRequired, "admission-recovery-bind", "recovery publication permit was already consumed", nil)
	}
	fsResult, transitionErr := p.mutation.BindPublishedObject(ctx, p.inventory, p.recoveryPublication, pre.candidateDigest, p.recoverySize)
	result := RecoveryBindingTransitionResult{outcome: fsResult.Outcome(), candidateDigest: fsResult.CandidateDigest(), candidateSequence: 4, candidateRevision: fsResult.CandidateRevision(), previousRevision: fsResult.PreviousRevision()}
	if fsResult.Outcome() != evidencefs.AdmissionTransitionDurable {
		if fsResult.Outcome() == evidencefs.AdmissionTransitionPreMutationFailure {
			p.consumed.CompareAndSwap(true, false)
		}
		return result, mapAdmissionMutationError(transitionErr, "admission-recovery-bind")
	}
	if transitionErr != nil || fsResult.CandidateKind() != "content_binding" || fsResult.CandidateSequence() != 0 || fsResult.CandidateDigest() != pre.candidateDigest || fsResult.Size() != p.recoverySize || fsResult.PreviousRevision() != pre.previousRevision || fsResult.CandidateRevision() != pre.candidateRevision || fsResult.Publication() != p.recoveryPublication || !fsResult.ValidFor(fsResult.Inventory()) {
		_ = fsResult.Invalidate()
		return recoveryBindingUnknown(result), admissionPostMutationFailure("admission-recovery-bind")
	}
	nextInventory := fsResult.Inventory()
	if err := nextInventory.Revalidate(ctx); err != nil {
		_ = fsResult.Invalidate()
		return recoveryBindingUnknown(result), admissionPostMutationFailure("admission-recovery-bind-revalidate")
	}
	revision, err := nextInventory.Revision()
	if err != nil || revision != pre.candidateRevision {
		_ = fsResult.Invalidate()
		return recoveryBindingUnknown(result), admissionPostMutationFailure("admission-recovery-bind-revision")
	}
	fullSet, err := nextInventory.FullSetDigest()
	if err != nil || fullSet != p.fullSet || !p.runtimePublication.Matches(digestRaw(p.runtimeDigest), p.runtimeSize) || !p.recoveryPublication.Matches(pre.candidateDigest, p.recoverySize) {
		_ = fsResult.Invalidate()
		return recoveryBindingUnknown(result), admissionPostMutationFailure("admission-recovery-bind-full-set")
	}
	nextToken, err := nextInventory.MutationToken()
	if err != nil || !nextToken.ValidFor(nextInventory) {
		_ = fsResult.Invalidate()
		return recoveryBindingUnknown(result), admissionPostMutationFailure("admission-recovery-bind-token")
	}
	next := &RecoveryBoundPermit{
		prior: p, plan: p.plan, candidateBinding: candidate.binding, inventory: nextInventory, mutation: nextToken,
		...
		recoveryDigest: p.recoveryDigest, recoverySize: p.recoverySize, consumed: &atomic.Bool{},
	}
	next.self = next
	binding := &recoveryBoundPermitBinding{
		permit: next, prior: p, plan: p.plan, inventory: nextInventory, mutation: nextToken,
	...
	next.binding = binding
	binding.canonical = recoveryBoundPermitDigest(next)
	recoveryBoundPermitRegistry.Store(binding, binding.canonical)
	if !validRecoveryBoundPermit(next, nextInventory, candidate) {
		_ = fsResult.Invalidate()
		return recoveryBindingUnknown(result), admissionPostMutationFailure("admission-recovery-bind-seal")
	}
	result.next = next
	return result, nil
}

func recoveryBindingUnknown(value RecoveryBindingTransitionResult) RecoveryBindingTransitionResult {
	value.outcome = evidencefs.AdmissionTransitionUnknown
	value.next = nil
	return value
}

func recoveryBoundPermitDigest(permit *RecoveryBoundPermit) [32]byte {
	if permit == nil || permit.self != permit || permit.prior == nil || permit.plan == nil || permit.candidateBinding == nil || permit.prior.binding == nil || permit.plan.binding == nil {
		return [32]byte{}
	}
	h := sha256.New()
	h.Write([]byte("cloud-agents-platform-recovery-bound-permit/v1\x00"))
	h.Write(permit.prior.binding.canonical[:])
	h.Write(permit.plan.binding.canonical[:])
	h.Write(permit.candidateBinding.canonical[:])
	h.Write(permit.target[:])
...
func validRecoveryBoundPermit(permit *RecoveryBoundPermit, inventory *evidencefs.AdmissionInventory, candidate OwnedCurrentCandidate) bool {
	if permit == nil || permit.self != permit || permit.binding == nil || permit.binding.permit != permit || permit.prior == nil || permit.plan == nil || permit.inventory != inventory || permit.mutation == nil || permit.runtimePublication == nil || permit.recoveryPublication == nil || permit.candidateBinding != candidate.binding || permit.binding.prior != permit.prior || permit.binding.plan != permit.plan || permit.binding.inventory != inventory || permit.binding.mutation != permit.mutation || permit.binding.runtimePublication != permit.runtimePublication || permit.binding.recoveryPublication != permit.recoveryPublication || permit.consumed == nil || permit.consumed.Load() || !validConsumedRecoveryPublishedPermit(permit.prior, permit.plan, candidate) || permit.revision != permit.prior.revision+1 || permit.target != permit.prior.target || permit.fullSet != permit.prior.fullSet || permit.runtimeDigest != candidate.runtimeArtifact.digest || permit.runtimeSize != candidate.runtimeArtifact.sizeBytes || permit.recoveryDigest != candidate.decisionRecoveryArtifact.digest || permit.recoverySize != candidate.decisionRecoveryArtifact.sizeBytes || !permit.runtimePublication.Matches(digestRaw(permit.runtimeDigest), permit.runtimeSize) || !permit.recoveryPublication.Matches(digestRaw(permit.recoveryDigest), permit.recoverySize) || !permit.mutation.ValidFor(inventory) || permit.binding.canonical == ([32]byte{}) || permit.binding.canonical != recoveryBoundPermitDigest(permit) {
		return false
	}
...
func validConsumedRecoveryPublishedPermit(permit *RecoveryPublishedPermit, plan *VerifiedAdmissionPlan, candidate OwnedCurrentCandidate) bool {
	if permit == nil || permit.self != permit || permit.binding == nil || permit.binding.permit != permit || permit.prior == nil || permit.plan != plan || plan == nil || permit.inventory == nil || permit.mutation == nil || permit.runtimePublication == nil || permit.recoveryPublication == nil || permit.candidateBinding != candidate.binding || permit.binding.prior != permit.prior || permit.binding.plan != plan || permit.binding.inventory != permit.inventory || permit.binding.mutation != permit.mutation || permit.binding.runtimePublication != permit.runtimePublication || permit.binding.recoveryPublication != permit.recoveryPublication || permit.consumed == nil || !permit.consumed.Load() || !validConsumedRuntimeBoundPermit(permit.prior, plan, candidate) || permit.revision != permit.prior.revision+1 || permit.target != permit.prior.target || (permit.fullSet == permit.prior.fullSet) != permit.reused || permit.runtimeDigest != candidate.runtimeArtifact.digest || permit.runtimeSize != candidate.runtimeArtifact.sizeBytes || permit.recoveryDigest != candidate.decisionRecoveryArtifact.digest || permit.recoverySize != candidate.decisionRecoveryArtifact.sizeBytes || !permit.runtimePublication.Matches(digestRaw(permit.runtimeDigest), permit.runtimeSize) || permit.fsResult.Publication() != permit.recoveryPublication || permit.fsResult.CandidateDigest() != digestRaw(permit.recoveryDigest) || permit.fsResult.Size() != permit.recoverySize || permit.binding.canonical != recoveryPublishedPermitDigest(permit) {
		return false
	}
	registered, ok := recoveryPublishedPermitRegistry.Load(permit.binding)
	return ok && registered == permit.binding.canonical
