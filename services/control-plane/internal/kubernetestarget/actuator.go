	...
	return []desiredResource{
		...
		{path: "/apis/apps/v1/namespaces/" + url.PathEscape(config.Namespace) + "/deployments/" + url.PathEscape(name), body: map[string]any{
			...
			"spec": map[string]any{"replicas": 1, "strategy": map[string]string{"type": "Recreate"}, "selector": map[string]any{"matchLabels": selector}, "template": map[string]any{
				...
				}}, "volumes": []map[string]any{{"name": "worker-credentials", "secret": map[string]string{"secretName": config.WorkerCredentialSecretRef}}, {"name": "provider-credentials", "secret": map[string]string{"secretName": request.ProviderCredentialRef}}, {"name": "workspace", "persistentVolumeClaim": map[string]string{"claimName": name}}, {"name": "tmp", "emptyDir": map[string]any{"medium": "Memory", "sizeLimit": "64Mi"}}}},
			}},
		}},
	}
}
