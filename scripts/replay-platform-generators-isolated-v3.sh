...
readonly WRAPPER_POLICY="VERSIONED_ISOLATION_WRAPPER_V3"
...
authority_runner_path() {
  printf '%s/replay-platform-generators-v3.ts\n' "$(cd "$(dirname "$(wrapper_path)")" && /bin/pwd -P)"
}
...
verify_authority_digests() {
  ...
  local runner="$root/scripts/replay-platform-generators-v3.ts"
...
process = subprocess.Popen(
    [bun, f"{root}/scripts/replay-platform-generators-v3.ts", "--run", root],
    cwd=root,
    env=child_env,
...
authority = {
    "wrapper": sha(wrapper),
    "runner": sha(authority_dir / "replay-platform-generators-v3.ts"),
