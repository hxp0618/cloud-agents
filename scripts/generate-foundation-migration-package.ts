...
const base = "services/control-plane/migrations/product/000059";
const sqlPath = "services/control-plane/migrations/000059_add_sandbox_file_access.sql";
...
const prior = json("services/control-plane/migrations/product/000058/manifest.json");
const catalog = json(prior.schema_bundle.migrations.at(-1).catalog_contract.path);
catalog.schema_head = "000059";
...
catalog.source_descriptors.push({
  ...
  statements: splitPostgresStatements(sql).map((s) => ({
    ...
    sha256: s.sha256,
    classification: classifyMigrationStatement(s, "000059"),
  })),
});
const additions: any[] = [];
for (const [name, arguments_] of [
  ["start_sandbox_file_access_v1", ["text", "text", "text", "text", "text", "text"]],
  ["complete_sandbox_file_access_v1", ["text", "text", "text", "text", "text", "text", "bigint"]],
] as const)
  additions.push({
    kind: "function",
    identity: {
...
const catalogArtifact = artifact(`${base}/catalog/schema-000058.json`, catalogBytes);
const schema = structuredClone(prior.schema_bundle);
schema.schema_head = "000059";
schema.migrations.push({
  ...structuredClone(schema.migrations.at(-1)),
  id: "000059",
  name: "add_sandbox_file_access",
  predecessor_id: "000058",
  schema_from: "000058",
  schema_to: "000059",
...
process.stdout.write(`product-000059 ${mode}: ${schemaDigest}\n`);
