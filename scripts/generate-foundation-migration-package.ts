...
const base = "services/control-plane/migrations/product/000058";
const sqlPath = "services/control-plane/migrations/000058_add_sandbox_access_grants.sql";
...
const prior = json("services/control-plane/migrations/product/000057/manifest.json");
const catalog = json(prior.schema_bundle.migrations.at(-1).catalog_contract.path);
catalog.schema_head = "000058";
...
catalog.source_descriptors.push({
  ...
  statements: splitPostgresStatements(sql).map((s) => ({
    ...
    sha256: s.sha256,
    classification: classifyMigrationStatement(s, "000058"),
  })),
});
const additions: any[] = [];
for (const [name, arguments_] of [
  [
    "issue_sandbox_access_grant_v1",
    ["text", "text", "bigint", "text", "text", "integer", "text", "text", "text", "text"],
  ],
  [
    "revoke_sandbox_access_grant_v1",
    ["text", "text", "text", "bigint", "bigint", "text", "text", "text", "text", "text"],
  ],
  ["register_sandbox_pty_session_v1", ["text", "text", "text", "text"]],
  ["delete_sandbox_pty_session_v1", ["text", "text", "text", "text"]],
] as const)
  additions.push({
    kind: "function",
    identity: {
...
const catalogArtifact = artifact(`${base}/catalog/schema-000057.json`, catalogBytes);
const schema = structuredClone(prior.schema_bundle);
schema.schema_head = "000058";
schema.migrations.push({
  ...structuredClone(schema.migrations.at(-1)),
  id: "000058",
  name: "add_sandbox_access_grants",
  predecessor_id: "000057",
  schema_from: "000057",
  schema_to: "000058",
...
process.stdout.write(`product-000058 ${mode}: ${schemaDigest}\n`);
