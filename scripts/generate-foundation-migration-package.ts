...
const base = "services/control-plane/migrations/product/000053";
const sqlPath = "services/control-plane/migrations/000053_add_foundation_intents.sql";
...
const prior = json("services/control-plane/migrations/product/000052/manifest.json");
const catalog = json(prior.schema_bundle.migrations.at(-1).catalog_contract.path);
catalog.schema_head = "000053";
...
catalog.source_descriptors.push({
  ...
  statements: splitPostgresStatements(sql).map((s) => ({
    ...
    sha256: s.sha256,
    classification: classifyMigrationStatement(s, "000053"),
  })),
});
const additions: any[] = [];
for (const name of ["workspaces", "workspace_volumes", "sandbox_sessions"]) {
...
for (const [name, relation] of [
  ["workspace_volumes_target_idx", "workspace_volumes"],
  ["sandbox_sessions_single_writer", "sandbox_sessions"],
  ["sandbox_sessions_workspace_idx", "sandbox_sessions"],
  ["sandbox_sessions_operation_idx", "sandbox_sessions"],
])
  additions.push({
    kind: "index",
...
additions.push({
  ...
  identity: {
    schema: "cloud_agents",
    name: "accept_foundation_intent_v1",
    arguments: [...Array(7).fill("text"), "bigint", "bigint", ...Array(5).fill("text")].map(
...
const catalogArtifact = artifact(`${base}/catalog/schema-000053.json`, catalogBytes);
const schema = structuredClone(prior.schema_bundle);
schema.schema_head = "000053";
schema.migrations.push({
  ...structuredClone(schema.migrations.at(-1)),
  id: "000053",
  name: "add_foundation_intents",
  predecessor_id: "000052",
  schema_from: "000052",
  schema_to: "000053",
...
process.stdout.write(`product-000053 ${mode}: ${schemaDigest}\n`);
