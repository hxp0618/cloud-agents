...
const base = "services/control-plane/migrations/product/000057";
const sqlPath = "services/control-plane/migrations/000057_add_foundation_sandbox_ttl.sql";
...
const prior = json("services/control-plane/migrations/product/000056/manifest.json");
const catalog = json(prior.schema_bundle.migrations.at(-1).catalog_contract.path);
catalog.schema_head = "000057";
...
catalog.source_descriptors.push({
  ...
  statements: splitPostgresStatements(sql).map((s) => ({
    ...
    sha256: s.sha256,
    classification: classifyMigrationStatement(s, "000057"),
  })),
});
const additions: any[] = [];
for (const [name, arguments_] of [
  ["accept_foundation_sandbox_v2", ["text", "text", "text", "text", "text", "bigint", "text", "text", "text", "integer"]],
  ["transition_foundation_sandbox_v2", ["text", "text", "bigint", "bigint", "text", "text", "text", "text", "text", "text", "text", "text"]],
  ["expire_foundation_sandbox_v1", ["text", "text"]],
  ["claim_foundation_sandbox_v3", ["text", "text", "text", "integer", "text", "text"]],
] as const)
  additions.push({
    kind: "function",
    identity: {
...
const catalogArtifact = artifact(`${base}/catalog/schema-000056.json`, catalogBytes);
const schema = structuredClone(prior.schema_bundle);
schema.schema_head = "000057";
schema.migrations.push({
  ...structuredClone(schema.migrations.at(-1)),
  id: "000057",
  name: "add_foundation_sandbox_ttl",
  predecessor_id: "000056",
  schema_from: "000056",
  schema_to: "000057",
...
process.stdout.write(`product-000057 ${mode}: ${schemaDigest}\n`);
