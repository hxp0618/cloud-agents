...
BEGIN
    ...
    VALUES (tenant, tenant, event_value, '@REGISTRY_DIGEST@', '@PROFILE_ID@', '@PROFILE_DIGEST@',
...
BEGIN
    ...
    WHERE event.tenant_id = p_tenant AND event.event_id = p_event AND event.state = 'claimed'
      AND event.profile_id = '@PROFILE_ID@' AND event.claim_holder_id = p_holder
...
BEGIN
    ...
    IF NOT FOUND OR event.state <> 'claimed' OR event.profile_id <> '@PROFILE_ID@' OR event.profile_digest <> '@PROFILE_DIGEST@'
