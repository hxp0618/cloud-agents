...
export const WORKER_LOCALDEV_LAUNCHER_SOURCE_PATH =
  "services/worker/localdev-launcher-profile/v1/authority-source.json";
export const WORKER_LOCALDEV_LAUNCHER_SOURCE_SCHEMA_PATH =
  "services/worker/localdev-launcher-profile/v1/authority-source.schema.json";
export const WORKER_LOCALDEV_LAUNCHER_PROFILE_PATH =
  "services/worker/localdev-launcher-profile/v1/profile.json";
export const WORKER_LOCALDEV_LAUNCHER_PROFILE_SCHEMA_PATH =
  "services/worker/localdev-launcher-profile/v1/profile.schema.json";
export const WORKER_LOCALDEV_LAUNCHER_GO_PATH =
  "services/worker/localdev_launcher_profile_generated.go";
export const WORKER_LOCALDEV_LAUNCHER_AUTHORITY_ID = "D-056-WORKER-LOCALDEV-LAUNCHER-000001";
export const WORKER_LOCALDEV_LAUNCHER_REVISION = `${WORKER_LOCALDEV_LAUNCHER_AUTHORITY_ID}.r1`;
export const WORKER_LOCALDEV_LAUNCHER_PROFILE_ID = "cloud-agents/worker-localdev-launcher/v1alpha1";

const SOURCE_SCHEMA =
  "https://schemas.cloud-agents.dev/worker/localdev-launcher/v1/authority-source.schema.json";
const PROFILE_SCHEMA =
  "https://schemas.cloud-agents.dev/worker/localdev-launcher/v1/profile.schema.json";
const SOURCE_FORMAT = "cloud-agents-worker-localdev-launcher-authority/v1";
const PROFILE_FORMAT = "cloud-agents-worker-localdev-launcher-profile/v1";
const SOURCE_DOMAIN = "cloud-agents/worker-localdev-launcher/source/v1";
const PROFILE_DOMAIN = "cloud-agents/worker-localdev-launcher/profile/v1";
...
const RECEIPT = {
  ...
  evidencePaths: [
    "docs/plan/standalone/worker-localdev-launcher-implementation-20260828.md",
    "docs/plan/standalone/worker-localdev-launcher-independent-review-20260828.md",
  ],
} as const;
const LINEAGE_FENCE = {
  kind: "single-predecessor-append-only",
  predecessorAuthority: "D-055-MANAGED-AGENT-WORKER-COORDINATION-000001.r1",
  predecessorRevision: "D-055-MANAGED-AGENT-WORKER-COORDINATION-000001.r1",
  predecessorProfile: "cloud-agents/managed-agent-worker-local-execution/localdev-v1alpha1",
  ...
  successorRevision: WORKER_LOCALDEV_LAUNCHER_REVISION,
} as const;
const REVIEW_RULES = {
  independentReadOnly: true,
  reviewPath: "docs/plan/standalone/worker-localdev-launcher-independent-review-20260828.md",
...
const SELECTOR = {
  ...
  tokenSource: "ephemeral_file_o_excl_mode_0600",
  ...
  generation: 1,
  routes: ["/healthz", "/cloudagents.worker.v1alpha1.WorkerExecutionService/"],
} as const;
const STATE_MACHINE = {
  initial: "starting",
...
export type WorkerLocalDevLauncherContract = JsonRecord & {
...
function base(root: string): JsonRecord {
  return {
    authorityId: WORKER_LOCALDEV_LAUNCHER_AUTHORITY_ID,
    revision: WORKER_LOCALDEV_LAUNCHER_REVISION,
    decision: WORKER_LOCALDEV_LAUNCHER_REVISION,
    profileId: WORKER_LOCALDEV_LAUNCHER_PROFILE_ID,
    mode: "localdev_only",
...
export function buildWorkerLocalDevLauncherSource(
  root: string = DEFAULT_ROOT,
): WorkerLocalDevLauncherContract {
...
export function buildWorkerLocalDevLauncherProfile(
  root: string = DEFAULT_ROOT,
): WorkerLocalDevLauncherContract {
  const s = buildWorkerLocalDevLauncherSource(root);
  const b = {
...
function schema(kind: "source" | "profile", root: string): JsonRecord {
  const value =
    kind === "source"
      ? buildWorkerLocalDevLauncherSource(root)
      : buildWorkerLocalDevLauncherProfile(root);
  return {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: kind === "source" ? SOURCE_SCHEMA : PROFILE_SCHEMA,
    title: `${WORKER_LOCALDEV_LAUNCHER_AUTHORITY_ID} ${kind}`,
    type: "object",
    additionalProperties: false,
...
export const buildWorkerLocalDevLauncherSourceSchema = (root: string = DEFAULT_ROOT) =>
  schema("source", root);
export const buildWorkerLocalDevLauncherProfileSchema = (root: string = DEFAULT_ROOT) =>
  schema("profile", root);
const serialize = (v: JsonRecord) => `${JSON.stringify(v, null, 2)}\n`;
export function serializeWorkerLocalDevLauncherGo(p: WorkerLocalDevLauncherContract): string {
  const c = p as any;
  const lines = [
    ...
    ``,
    `const (`,
    `\tWorkerLocalDevLauncherProfileID = ${JSON.stringify(c.profileId)}`,
    `\tWorkerLocalDevLauncherAuthorityID = ${JSON.stringify(c.authorityId)}`,
    `\tWorkerLocalDevLauncherRevision = ${JSON.stringify(c.revision)}`,
    `\tWorkerLocalDevLauncherProfileDigest = ${JSON.stringify(c.profileDigest)}`,
    `\tWorkerLocalDevLauncherSourceDigest = ${JSON.stringify(c.sourceAuthority.sourceDigest)}`,
    `\tWorkerLocalDevLauncherInputManifestDigest = ${JSON.stringify(c.inputManifestDigest)}`,
    `\tWorkerLocalDevLauncherHTTPRoutePrefix = ${JSON.stringify("/cloudagents.worker.v1alpha1.WorkerExecutionService/")}`,
    `\tWorkerLocalDevLauncherHealthRoute = ${JSON.stringify("/healthz")}`,
    `\tWorkerLocalDevLauncherWorkerIdentitySPIFFE = ${JSON.stringify(SELECTOR.workerIdentitySPIFFE)}`,
    `\tWorkerLocalDevLauncherSupervisorIdentitySPIFFE = ${JSON.stringify(SELECTOR.supervisorIdentitySPIFFE)}`,
    `\tWorkerLocalDevLauncherLeaseID = ${JSON.stringify(SELECTOR.leaseID)}`,
    `\tWorkerLocalDevLauncherGeneration = 1`,
    `)`,
    ``,
    `type WorkerLocalDevLauncherProfile struct { ID, AuthorityID, Revision, ProfileDigest, SourceDigest, InputManifestDigest string; Mode, Transport string; RoutePrefix, HealthRoute, WorkerIdentitySPIFFE, SupervisorIdentitySPIFFE, LeaseID string; Generation uint64; ExternalSideEffects bool; CompleteLedger, EntryWriter, RecoveryWriter, DatabaseWrites, DurablePersistence, Provider, Runtime, ProductionHTTP, PublicHTTP, P2, Deployment, Publication, GateTransition string }`,
    ``,
    ...
    ``,
    `func (p WorkerLocalDevLauncherProfile) Valid() bool { return p == GeneratedWorkerLocalDevLauncherProfile }`,
    ``,
    `func WorkerLocalDevLauncherProfileAuthority() WorkerLocalDevLauncherProfile { return GeneratedWorkerLocalDevLauncherProfile }`,
    ``,
  ].join("\n");
...
export function assertWorkerLocalDevLauncherCurrent(root: string = DEFAULT_ROOT): void {
  assertDeclared(root);
  const s = buildWorkerLocalDevLauncherSource(root),
    p = buildWorkerLocalDevLauncherProfile(root);
  const ss = buildWorkerLocalDevLauncherSourceSchema(root),
    ps = buildWorkerLocalDevLauncherProfileSchema(root);
  const ajv = new Ajv2020({ strict: true, allErrors: true, validateFormats: false });
  if (!ajv.compile(ss)(parse(root, WORKER_LOCALDEV_LAUNCHER_SOURCE_PATH)))
    throw new Error("source schema validation failed");
  if (!ajv.compile(ps)(parse(root, WORKER_LOCALDEV_LAUNCHER_PROFILE_PATH)))
    throw new Error("profile schema validation failed");
  assertExact(root, WORKER_LOCALDEV_LAUNCHER_SOURCE_PATH, serialize(s));
  assertExact(root, WORKER_LOCALDEV_LAUNCHER_PROFILE_PATH, serialize(p));
  assertExact(root, WORKER_LOCALDEV_LAUNCHER_SOURCE_SCHEMA_PATH, serialize(ss));
  assertExact(root, WORKER_LOCALDEV_LAUNCHER_PROFILE_SCHEMA_PATH, serialize(ps));
  assertExact(root, WORKER_LOCALDEV_LAUNCHER_GO_PATH, serializeWorkerLocalDevLauncherGo(p));
}
export function writeWorkerLocalDevLauncherFiles(root: string = DEFAULT_ROOT): void {
  assertDeclared(root);
  const s = buildWorkerLocalDevLauncherSource(root),
    p = buildWorkerLocalDevLauncherProfile(root),
    ss = buildWorkerLocalDevLauncherSourceSchema(root),
    ps = buildWorkerLocalDevLauncherProfileSchema(root);
  const files: [string, string][] = [
    [WORKER_LOCALDEV_LAUNCHER_SOURCE_PATH, serialize(s)],
    [WORKER_LOCALDEV_LAUNCHER_PROFILE_PATH, serialize(p)],
    [WORKER_LOCALDEV_LAUNCHER_SOURCE_SCHEMA_PATH, serialize(ss)],
    [WORKER_LOCALDEV_LAUNCHER_PROFILE_SCHEMA_PATH, serialize(ps)],
    [WORKER_LOCALDEV_LAUNCHER_GO_PATH, serializeWorkerLocalDevLauncherGo(p)],
  ];
...
export const workerLocalDevLauncherSourceDigest = (root: string = DEFAULT_ROOT) =>
  buildWorkerLocalDevLauncherSource(root).sourceDigest as Digest;
export const workerLocalDevLauncherProfileDigest = (root: string = DEFAULT_ROOT) =>
  buildWorkerLocalDevLauncherProfile(root).profileDigest as Digest;
export const isWorkerLocalDevLauncherDigest = (v: unknown): v is Digest =>
