...
export const DURABLE_PROJECT_CREATE_SUCCESSOR_SQL_PATH = `${ROOT}/000014_harden_durable_project_create_identifiers.sql`;
export const DURABLE_PROJECT_CREATE_SUCCESSOR_CATALOG_PATH = `${ROOT}/catalog/schema-000014.json`;
const SQL_PATHS = [
...
export function validateCatalogStatementBindings(
  ...
  if (sqlBytes.has(DURABLE_PROJECT_CREATE_SUCCESSOR_SQL_PATH)) {
    expectedSourcesByHead.set(
      "000014",
      generatedCatalogs.get(DURABLE_PROJECT_CREATE_SUCCESSOR_CATALOG_PATH)!.source_descriptors!,
    );
  }
  ...
  if (sqlBytes.has(DURABLE_PROJECT_CREATE_SUCCESSOR_SQL_PATH)) {
    declaredByHead.set("000014", DECLARED_IDENTITIES_000013);
  }
...
export function migrationStatementSourceDescriptors(
  ...
  return migrationStatementSourceDescriptorsForPaths(sqlBytes, SQL_PATHS);
}

export function migrationStatementSourceDescriptorsForPaths(
  sqlBytes: ReadonlyMap<string, Uint8Array>,
  paths: ReadonlyArray<string>,
): MigrationJson[] {
  return paths.map((path, migrationIndex) => ({
...
/**
 * Build only the cumulative catalog for the append-only 000014 identifier
 * successor.  The historical bundle remains on its 000013 path; callers use
 * this helper to materialize the successor authority without changing that
 * predecessor's generated bytes.
 */
export function buildDurableProjectCreateSuccessorCatalog(root: string): JsonObject {
  const paths = [...SQL_PATHS, DURABLE_PROJECT_CREATE_SUCCESSOR_SQL_PATH];
  const sqlBytes = new Map(paths.map((path) => [path, readExactFile(root, path)] as const));
  const projection = buildProjectionDocuments(sqlBytes);
  const catalog = projection.catalogDocuments.get(DURABLE_PROJECT_CREATE_SUCCESSOR_CATALOG_PATH);
  if (!catalog) {
    throw new MigrationValidationError(
      "SUCCESSOR_CATALOG_MISSING",
...
function buildProjectionDocuments(sqlBytes: ReadonlyMap<string, Uint8Array>): ProjectionDocuments {
  const successorPaths = sqlBytes.has(DURABLE_PROJECT_CREATE_SUCCESSOR_SQL_PATH)
    ? [...SQL_PATHS, DURABLE_PROJECT_CREATE_SUCCESSOR_SQL_PATH]
    : SQL_PATHS;
  const rawSources = migrationStatementSourceDescriptorsForPaths(sqlBytes, successorPaths);
  ...
  const schema000013 = contract("000013", rawSources.slice(0, 13), declared000013);
  const schema000014 =
    successorPaths.length === 14 ? contract("000014", rawSources, declared000013) : undefined;
  ...
  if (schema000014 !== undefined) {
    catalogDocuments.set(DURABLE_PROJECT_CREATE_SUCCESSOR_CATALOG_PATH, schema000014);
  }
