...
function validateProtoDescriptorExpectation(
  ...
  if (
    descriptor.generationStatus !== "GENERATED_NON_GATE_EVIDENCE" ||
    descriptor.authority !== "proto3"
  ) {
    throw new Error(`${descriptorFile} must bind generated non-Gate Proto evidence.`);
  }
  const descriptorSet = requiredString(descriptor.descriptorSet, `${descriptorFile} descriptorSet`);
  const breakingBaseline = requiredString(
    descriptor.breakingBaseline,
    `${descriptorFile} breakingBaseline`,
  );
  for (const path of [descriptorSet, breakingBaseline]) {
    if (!path.startsWith("contracts/generated/proto/") || !path.endsWith(".binpb")) {
      throw new Error(`${descriptorFile} has invalid generated descriptor path ${path}.`);
    }
    const target = resolve(root, path);
    const stat = lstatSync(target);
    if (!stat.isFile() || stat.isSymbolicLink()) {
      throw new Error(`${descriptorFile} descriptor ${path} must be a regular file.`);
    }
  }
  const descriptorDigest = `sha256:${createHash("sha256")
    .update(readFileSync(resolve(root, descriptorSet)))
    .digest("hex")}`;
  if (descriptor.descriptorSha256 !== descriptorDigest) {
    throw new Error(`${descriptorFile} descriptor digest does not match generated bytes.`);
  }
  if (
    !readFileSync(resolve(root, descriptorSet)).equals(
      readFileSync(resolve(root, breakingBaseline)),
    )
  ) {
    throw new Error(`${descriptorFile} current descriptor breaks its fixed v1alpha1 baseline.`);
  }
  assertStringArrayEqual(
    requiredArray(descriptor.protocolCompatibility, `${descriptorFile} protocolCompatibility`).map(
      (value) => requiredString(value, `${descriptorFile} protocolCompatibility entry`),
    ),
    ["connect", "grpc"],
    `${descriptorFile} protocolCompatibility`,
  );
