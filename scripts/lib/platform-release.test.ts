...
describe("platform release", () => {
  ...
  it("packages the current product migration manifest, catalog, and SQL", () => {
    ...
    expect(entries.some(({ path }) => path.endsWith("product/000086/manifest.json"))).toBe(true);
    expect(entries.filter(({ path }) => path.endsWith(".sql"))).toHaveLength(86);
    expect(PLATFORM_RELEASE_MIGRATION_HEAD).toBe("000086");
    expect(expectedArtifactIdentities()).toContainEqual({
  ...
  it("packages the independent Compose and Helm deployment inputs", () => {
    ...
    expect(entries.map(({ path }) => path)).toEqual([
      "LICENSE",
      "deploy/admin-web/dist/assets/index-BpQrCaLy.js",
  ...
  it("selects matching OCI base and binary architectures", () => {
    ...
    expect(migrateDockerfile).toContain("cloud-agents-migrations-000086.tar");
    expect(migrateDockerfile).toContain("product/000086/manifest.json");
