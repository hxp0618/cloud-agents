...
expect_startup_failure migrate 1 "database URL, repository root, and product-000036 selector are required"
...
docker run --rm --entrypoint /usr/bin/test "$image_prefix:migrate" \
  -r /opt/cloud-agents/migrations/services/control-plane/migrations/product/000036/manifest.json
